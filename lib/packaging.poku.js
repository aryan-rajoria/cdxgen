...
describe("npm pack file list", () => {
  it("matches the release/13.0.x baseline exactly", () => {
    const current = getPackFileList();
    const baseline = getBaselineFileList();
...
describe("exports map resolution", () => {
  ...
  it("every exports subpath resolves via import.meta.resolve", () => {
    ...
    for (const entry of entries) {
      ...
      // specifier becomes "@cyclonedx/cdxgen/evinser/db" or "@cyclonedx/cdxgen"
      const specifier =
        entry.subpath === "."
          ? packageName
      ...
      try {
        ...
        for (const entry of entries) {
          const subpath =
            entry.subpath === "."
              ? "@cyclonedx/cdxgen"
              : `@cyclonedx/cdxgen${entry.subpath.slice(1)}`;
          try {
