...
describe("parsePyLockData resilience to an unparseable workspace pyproject.toml", () => {
  // cdxgen's TOML parser (@iarna/toml 2.2.5) enforces TOML 0.5's "inline lists must be
  // a single type" rule, which TOML 1.0 removed. PEP 735 dependency groups mix an
  // inline table (`include-group`) with strings, so real projects carry a
  // pyproject.toml this parser rejects - pydantic's vendored `pydantic-core` is one.
  // When such a file is a *workspace member*, the member's parse returns no parent
  // component, and reading its `bom-ref` anyway threw and took the whole SBOM down.
