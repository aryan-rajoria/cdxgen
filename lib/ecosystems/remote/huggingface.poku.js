...
import { quantizationValueFromConfig } from "../../parsers/huggingfaceUtils.js";
import {
...
describe("huggingface remote helper", () => {
  ...
  it("keeps ambiguous repo-id shapes unresolved in explicitOnly mode", () => {
    // Bare `owner/repo` values collide with one-slash relative paths such as
    // `repotests/sveltejs-realworld`, so CLI source paths only resolve
    // unambiguous references until an AI inventory type is selected (#4358).
    assert.strictEqual(
      normalizeHuggingFaceReference("repotests/sveltejs-realworld", {
        explicitOnly: true,
      }),
      undefined,
    );
    ...
    assert.strictEqual(
      normalizeHuggingFaceReference("models/openai/whisper-small", {
        explicitOnly: true,
      }),
      undefined,
    );
    // Explicit purls and huggingface.co URLs stay resolvable.
    assert.deepStrictEqual(
      normalizeHuggingFaceReference("pkg:huggingface/openai/whisper-small", {
        explicitOnly: true,
      }),
      {
        assetType: "model",
  ...
  it("resolves Hugging Face spaces into application components with model and dataset dependencies", async () => {
    ...
    assert.deepStrictEqual(inventory.dependencies, [
      {
        ref: "pkg:huggingface/team/demo-space@space123?repository_url=https:%2F%2Fhuggingface.co%2Fspaces",
        dependsOn: [
          "pkg:huggingface/HuggingFaceH4/ultrachat_200k?repository_url=https:%2F%2Fhuggingface.co%2Fdatasets",
