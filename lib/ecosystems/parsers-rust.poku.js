...
it("annotate cargo build and target excluded components", () => {
  ...
  // A build dependency is provided by the build environment, not bundled.
  assert.deepStrictEqual(pkgList[1].isExternal, true);
  assert.deepStrictEqual(pkgList[1].scope, undefined);
  // A proc-macro is a host artefact even without a rollup entry.
  assert.deepStrictEqual(pkgList[2].isExternal, true);
  ...
  assert.deepStrictEqual(pkgList[0].scope, undefined);
  assert.deepStrictEqual(pkgList[0].isExternal, undefined);
});
