...
function normalizeReference(ref) {
  ...
  if (typeof ref.purl === "string" && ref.purl.length) {
    try {
      const purl = Purl.parse(ref.purl);
      return {
...
export function enrichComponentWithMcpMetadata(component) {
  ...
  // cdx:mcp:package is the retention signal for `--exclude-type mcp`: it is
  // stamped exclusively on genuine MCP SDK dependency packages (never on MCP
  // primitives, services, or config files), so matchesAiInventoryExcludeType()
  // uses it to keep real supply-chain components while dropping the inventory
  // overlay. Do not remove/rename it without updating that predicate.
