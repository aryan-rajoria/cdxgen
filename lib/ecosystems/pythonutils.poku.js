...
describe("isFirstPartyModule()", () => {
  const proj = createMockEnv("first_party_proj", {
    "config.py": "",
    "app/__init__.py": "",
    "cli/__main__.py": "",
    "plugins/thing.py": "",
  ...
  it("recognises a module file, a package, and a __main__-only package", () => {
    assert.deepStrictEqual(isFirstPartyModule(proj, "config"), true);
    assert.deepStrictEqual(isFirstPartyModule(proj, "app"), true);
    assert.deepStrictEqual(isFirstPartyModule(proj, "cli"), true);
  });

  it("recognises a PEP 420 namespace package with no __init__.py", () => {
    assert.deepStrictEqual(isFirstPartyModule(proj, "plugins"), true);
  });

  it("recognises a src/ layout package", () => {
    assert.deepStrictEqual(isFirstPartyModule(proj, "pkgname"), true);
  });

  it("does not claim a third-party distribution the project merely imports", () => {
    // The whole point: `flask` and `yaml` must still be attributed to pypi, or the
    // filter would delete real dependencies from the SBOM.
    for (const name of ["flask", "yaml", "requests", "pydantic"]) {
      assert.deepStrictEqual(isFirstPartyModule(proj, name), false, name);
    }
  });

  it("does not treat a directory without python sources as a module", () => {
    assert.deepStrictEqual(isFirstPartyModule(proj, "notes"), false);
  });

  it("returns false for unusable input and refuses path-shaped names", () => {
    assert.deepStrictEqual(isFirstPartyModule(proj, ""), false);
    assert.deepStrictEqual(isFirstPartyModule("", "app"), false);
    assert.deepStrictEqual(isFirstPartyModule(proj, undefined), false);
    assert.deepStrictEqual(isFirstPartyModule(proj, "../app"), false);
    assert.deepStrictEqual(isFirstPartyModule(proj, "a/b"), false);
  });
});
