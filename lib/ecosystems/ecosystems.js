...
import { isGitHubUrl, modulePathHost } from "../core/urls.js";
import {
  collectJarNS,
...
export async function getMvnMetadata(
  ...
  if (batchUrls.length) {
    ...
    for (const [key, value] of fetched) {
      ...
      ownPomUrls.push(key);
      const scmUrl = pomScmUrlNeedingLicenseLookup(value);
      if (scmUrl) {
        repoUrls.push(scmUrl);
      }
    }
    // The POMs are in hand, so the licence fallback's repository URLs are known
    // without a second network round for the POMs themselves — the same second
    // round getNpmMetadata does. Only a POM that declares no licence and no
    // licence comment reaches getRepoLicense, and only its own scm element is
    // read here, so this never fetches a licence the serial path would not.
    await prefetchRepoLicenses(repoUrls);
  }
  try {
...
/**
 * The repository URL a prefetched POM will need a GitHub licence lookup for.
 *
 * Mirrors the order `enrichMvnPackages` resolves a licence in: a `<licenses>`
 * element wins, then a licence comment, and only a POM offering neither falls
 * through to `getRepoLicense`. A POM whose `scm` element is inherited from a
 * parent is not covered, since the parent document is not prefetched.
 *
 * @param {{ok?: boolean, body?: String}} entry A prefetched POM batch entry.
 * @returns {String|undefined} The scm URL to prefetch a licence for, if any.
 */
function pomScmUrlNeedingLicenseLookup(entry) {
  if (!entry?.ok || !entry.body) {
    return undefined;
  }
  let pomJson;
  try {
    pomJson = xml2js(entry.body, {
      compact: true,
      spaces: 4,
      textKey: "_",
      attributesKey: "$",
  ...
  if (parseLicenseEntryOrArrayFromPomXml(pomJson?.licenses?.license)) {
    return undefined;
  }
  const commentMatch = /<!--([\s\S]*?)-->[\s\n]*<project/m.exec(entry.body);
  if (commentMatch?.[1] && findLicenseId(commentMatch[1].trim())) {
    return undefined;
  }
  return pomJson?.scm?.url?._;
}
...
export async function fetchPomXml({ urlPrefix, group, name, version }) {
  ...
  // through to the serial request. That fallback still reaches Maven Central
  // and still counts against the same budget as the batch round, so it queues
  // behind the same per-host gate rather than a second, ungoverned one.
  try {
    const res =
      prefetchedResponse(prefetchedPoms, fullUrl) ||
      (await withHostRateLimit(fullUrl, () => cdxgenAgent.get(fullUrl)));
    return res.body;
  } catch (_err) {
    return undefined;
  }
  ...
  for (const p of pkgList) {
    try {
      ...
      const res =
        ...
        (await withHostRateLimit(crateUrl, () =>
      ...
      try {
        ...
        ownersRes =
          ...
          (await withHostRateLimit(ownersUrl, () =>
 ...
 */
function applyCargoCacheMetadata(p) {
  const local = readCargoCacheMetadata(p.name, p.version);
  if (!local?.license) {
    return false;
  }
      ...
      if (jarResult.status === 0) {
        ...
        if (
          ...
          try {
            ...
            // uses (500 ms, serial) instead of a second, ungoverned path.
            const res = await withHostRateLimit(searchurl, () =>
