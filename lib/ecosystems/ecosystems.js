...
import { encodeForPurl } from "../inventory/purl.js";
import {
  findLicenseId,
  guessLicenseId,
  spdxLicenses,
...
export async function prefetchRepoLicenses(repoUrls) {
  ...
  for (const repoUrl of repoUrls) {
    ...
    requests.push({
      ...
      authRealm: process.env.GITHUB_TOKEN ? `github-auth:${apiUrl}` : undefined,
    });
  }
  if (!requests.length) {
    return;
...
export async function getNugetMetadata(pkgList, dependencies = undefined) {
  ...
  try {
    ...
    // process serves many scans.
    prefetchedNugetIndex.clear();
  }
