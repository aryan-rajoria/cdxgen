...
import { DEBUG_MODE, isDryRun, recordActivity } from "../core/activity.js";
import {
  getTmpDir,
  safeCopyFileSync,
  safeExistsSync,
...
function createAsarPackagePurl(packageName, version) {
  ...
  // Names come from an untrusted archive header, so a single unrepresentable
  // one must not abort the scan.
  return tryBuildPurl({
    type: "npm",
    namespace: parsedName.group || null,
...
function createGenericArchivePurl(archivePath, version) {
  // `generic` allows only checksum/download_url/repository_url/vcs_url as
  // qualifiers, so the old `?type=asar` made cdx-purl throw and aborted the
  // whole ASAR scan. The archive kind is already recorded as the
  // `cdx:file:kind=asar-archive` property, so nothing is lost by dropping it.
  return tryBuildPurl({
    type: "generic",
    name: basename(archivePath, ".asar"),
    version: version || null,
  });
}
 ...
 */
export function isAsarArchiveSync(archivePath) {
  try {
    ...
    try {
      ...
      const firstUInt = sizeBuffer.readUInt32LE(0);
      if (firstUInt !== 4) {
        return false;
      }
...
export function readAsarArchiveHeaderSync(archivePath) {
  ...
  try {
    ...
    if (firstUInt !== 4) {
      throw new Error(
        `Invalid ASAR header size check (expected 4, got ${firstUInt}) for ${archivePath}`,
      );
    }
    const headerPickleSize = sizeBuffer.readUInt32LE(PICKLE_UINT32_SIZE);
    if (
...
function createAsarSigningComponent(archivePath, signingInfo, options = {}) {
  ...
  const componentType =
    specVersionNumber > 0 && specVersionNumber < 1.6
      ? "data"
      : "cryptographic-asset";
  const properties = [{ name: "internal:SrcFile", value: archivePath }];
