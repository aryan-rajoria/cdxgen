...
/**
 * Collect the goldens of every scenario under a corpus root.
 *
 * A scenario is a directory holding a `golden.manifest.json`, the same rule
 * `contrib/golden-runner.js` discovers by. Anything else that lands in the
 * corpus root is scan input rather than cdxgen output, and asserting cdxgen's
 * own purl rules against it would fail on documents cdxgen never wrote.
 */
function collectCorpus(root) {
  ...
  for (const entry of entries) {
    ...
    const scenario = path.join(root, entry.name);
    if (!existsSync(path.join(scenario, "golden.manifest.json"))) {
      continue;
    }
    collectGoldens(scenario, acc);
  }
  return acc;
}
...
describe("committed goldens contain only valid purls", () => {
  const goldens = GOLDEN_DIRS.flatMap((d) =>
    collectCorpus(path.join(REPO_ROOT, d)),
  );
