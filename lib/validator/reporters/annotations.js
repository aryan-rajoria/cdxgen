...
function buildFindingDetails(f) {
  const details = [
    `Severity: ${f.severity || "unknown"}`,
    f.category ? `Category: ${f.category}` : undefined,
    f.status ? `Status: ${f.status}` : undefined,
    f.mitigation ? `Mitigation: ${f.mitigation}` : undefined,
  ];
  if (f.evidence && typeof f.evidence === "object") {
    for (const [key, value] of Object.entries(f.evidence).slice(0, 6)) {
      details.push(
        `Evidence — ${key}: ${typeof value === "object" ? JSON.stringify(value) : String(value)}`,
      );
    }
  }
...
export function buildAnnotations(findings, bomJson) {
  ...
  // An annotation describes the document it is attached to, so it carries that
  // document's timestamp. Annotating the same BOM therefore yields the same
  // annotations however long after generation it happens.
  const timestamp = bomJson.metadata?.timestamp || getTimestamp();
  return findings.map((f) => {
    ...
    return {
      ...
      timestamp,
      text: buildAnnotationText(f.message, properties, buildFindingDetails(f)),
    };
  });
}
