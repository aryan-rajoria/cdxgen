...
export const validatePurls = (bomJson) => {
  ...
  // A flat cap of 20 was a proxy for "the framework heuristic has over-fired", but it
  // scales with the size of the dependency tree rather than with any defect: a modern
  // Python or JavaScript application legitimately depends on dozens of components that
  // `data/frameworks-list.json` and the PyPI `Framework ::` classifiers correctly
  // identify as frameworks. Comparing against a share of the tree keeps the check
  // meaningful for small BOMs, where a high framework count really is suspicious, and
  // stops it firing on large ones purely for being large.
  const componentsCount = bomJson?.components?.length || 0;
  const frameworksLimit = Math.max(20, Math.ceil(componentsCount * 0.25));
  if (frameworksCount > frameworksLimit) {
    warningsList.push(
      `BOM likely has too many framework components. Count: ${frameworksCount} of ${componentsCount} components.`,
    );
  }
  if (DEBUG_MODE && warningsList.length !== 0) {
    console.log("===== WARNINGS =====");
