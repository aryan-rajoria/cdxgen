 ...
 * Validate a BOM.
 *
 * Validation is done in JavaScript. `cdxrs validate` is deliberately not used:
 * it rejects BOMs the JS validator accepts and reports its findings on stdout
 * rather than through the bridge, which turns a disagreement between the two
 * implementations into an unexplained non-zero exit.
 *
 * **No failure mode may abort SBOM generation.**
 *
 * @param {object} bomJson Parsed CycloneDX BOM.
 * @returns {Promise<{ valid: boolean, source: "js", findings: null }>}
 */
export async function validateGeneratedBom(bomJson) {
 ...
 *
 * An exception is not a verdict on the BOM, so it is recorded in
 * `validationErrors` instead of silently counting as a plain failure.
 *
 * @param {object} bomJson
 * @param {object} opts
 * @returns {Promise<{ schemaValid: boolean, deepValid: boolean, validationErrors: Array<{ stage: string, message: string }> }>}
 */
async function runSchemaAndDeep(bomJson, opts) {
  ...
  return { schemaValid, deepValid, validationErrors };
}
...
export async function validateBomAdvanced(bomJson, options = {}) {
  const { schemaValid, deepValid, validationErrors } = await runSchemaAndDeep(
    bomJson,
    options,
  );
