...
import { cdxrsAvailable, runCdxrs } from "../inventory/cdxrs.js";
import {
  validateBom,
  validateMetadata,
  validateProps,
 ...
 */
export async function validateBomWithRustFallback(bomJson) {
  // Try Rust path
  if (cdxrsAvailable("validate").available) {
    try {
      const result = await runCdxrs("validate", {
        content: JSON.stringify(bomJson),
        timeoutMs: 60_000,
      });
      if (result.ok) {
      ...
      thoughtLog(`Rust validate failed (${err?.message}), falling back to JS.`);
    }
  }

  // Fallback: JS validator
  const valid = (await validateBom(bomJson)) !== false;
  return { valid, source: "js", findings: null };
}
 ...
 * @param {object} opts
 * @returns {Promise<{ schemaValid: boolean, deepValid: boolean }>}
 */
async function runSchemaAndDeep(bomJson, opts) {
  // Try Rust path first: cdxrs validate combines schema + deep validation.
  if (cdxrsAvailable("validate").available) {
    try {
      const result = await runCdxrs("validate", {
        content: JSON.stringify(bomJson),
      ...
      if (result.ok) {
        const doc = JSON.parse(result.stdout);
        // cdxrs does schema and deep checks in one pass, but callers can switch
        // either off (`bin/validate.js --no-deep`, `--schema false`), so the two
        // flags must be derived separately rather than collapsed into one.
        //
        // Mirror the JS path's semantics exactly, including its quirk:
        // `validateBom` is not schema-only — it runs the schema check *and then*
        // all four deep checks (bomValidator.js), so `schemaValid` there means
        // "schema and deep both passed". `deepValid` is the deep checks alone.
        // Hence: schemaValid keys off *any* error finding, deepValid only off
        // non-`schema.*` ones. A switched-off check leaves its flag `true`,
        // matching JS, which simply does not run it.
        const errors = (doc.findings || []).filter(
          (f) => f.severity === "error",
        );
        const anyError = errors.length > 0;
        const deepError = errors.some(
          (f) => !String(f.id || "").startsWith("schema."),
        );
        return {
          schemaValid: opts.schema === false ? true : !anyError,
          deepValid: opts.deep === false ? true : !deepError,
        };
      }
    } catch {
  ...
  if (opts.schema !== false) {
    try {
      schemaValid = (await validateBom(bomJson)) !== false;
    } catch (err) {
      schemaValid = false;
 ...
 * }}
 */
export async function validateBomAdvanced(bomJson, options = {}) {
  const { schemaValid, deepValid } = await runSchemaAndDeep(bomJson, options);
