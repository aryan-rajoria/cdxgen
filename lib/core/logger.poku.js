...
// The probe is a module file so every runtime can load it: deno additionally
// needs the run subcommand, the repo config and an explicit permission set.
...
it("writes trace records to a log file and terminates the think block", async () => {
  ...
  try {
    execFileSync(process.execPath, probeArgv(probePath), {
      env: {
        ...process.env,
