...
import { retrieveCdxgenPluginVersion } from "./core-misc-b.js";
...
export function resolveCdxgenPlugins() {
  ...
  if (!pluginsDir) {
    // The plugins directory always ships inside the *platform-specific*
    // package (`@cdxgen/cdxgen-plugins-bin${pluginsBinSuffix}`), which stays
    // optional because it only resolves on platforms we publish a binary for.
    // An empty `pluginsBinSuffix` means this platform/arch has no published
    // package at all, so an absent plugins directory is expected there and
    // must not be reported as a broken install.
    if (target.pluginsBinSuffix) {
      console.warn(
        `@cdxgen/cdxgen-plugins-bin${target.pluginsBinSuffix} was not found. A binary package is published for this platform, so this is likely an install-integrity issue - reinstall cdxgen without excluding optional dependencies. Plugin-backed features will be skipped.`,
      );
    } else if (DEBUG_MODE) {
      console.log(
        `No cdxgen-plugins-bin package is published for ${target.platform}/${target.arch}; plugin-backed features will be skipped.`,
      );
    }
    pluginsDir = "";
  }
