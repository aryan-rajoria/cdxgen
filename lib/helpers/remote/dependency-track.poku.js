...
describe("Dependency-Track helper tests", () => {
  ...
  it("builds payload with parentUUID and comma separated tags", () => {
    ...
    assert.deepStrictEqual(payload, {
      ...
      projectName: "child",
      // The form endpoint splits this field on commas. Sending a JSON array here
      // creates tags named after the JSON fragments.
  ...
  it("sends every field as a string, including autoCreate and isLatest", () => {
    const payload = buildDependencyTrackBomPayload({
      autoCreate: false,
      isLatest: true,
      projectName: "child",
  ...
  it("uses the form field name isLatest rather than the JSON name", () => {
    const payload = buildDependencyTrackBomPayload({
      isLatest: "true",
      projectName: "child",
    });
    assert.strictEqual(payload.isLatest, "true");
    assert.strictEqual(payload.isLatestProjectVersion, undefined);
  });

  it("defaults projectVersion to main when only projectName is provided", () => {
    assert.deepStrictEqual(
      buildDependencyTrackBomPayload({ projectName: "child" }),
      {
        autoCreate: "true",
        projectName: "child",
        projectVersion: "main",
  ...
  it("carries the BOM verbatim instead of base64 encoding it", () => {
    const bomContents = {
      bomFormat: "CycloneDX",
      components: [{ name: "ünïcode-pkg", version: "1.0.0" }],
    };
    const request = buildDependencyTrackBomRequest(
      { projectName: "child", projectTag: "tag1", projectVersion: "1.0.0" },
      bomContents,
    );
    const parts = parseRequest(request);
    ...
    assert.strictEqual(parts.projectTags.value, "tag1");
    // Base64 would add a third to the BOM size; multipart only adds the part headers.
    assert.ok(
      request.body.length <
        Buffer.byteLength(JSON.stringify(bomContents)) * 1.1 + 1024,
    );
  });
