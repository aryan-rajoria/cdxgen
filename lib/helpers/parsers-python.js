...
function createPurlTemplate(packageData) {
  // conda defines build/channel/subdir/type as its qualifiers — `os` is not one
  // of them, so the previous `?os=` form produced an invalid purl for every pixi
  // package. The pixi field is already called subdir and maps straight across.
  return `pkg:${packageData["kind"]}/${packageData["name"]}@${packageData["version"]}-${packageData["build"]}?subdir=${packageData["subdir"]}`;
}
