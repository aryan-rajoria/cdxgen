...
describe("extractVsixToTempDir()", () => {
  it("returns undefined when dry-run blocks vsix extraction", async () => {
    ...
    const { extractVsixToTempDir } = await esmock("./vsixutils.js", {
      ...
      "../core/paths.js": { isMac: false, isWin: false },
    });
...
describe("toComponent", () => {
  ...
  it("should include srcPath in properties", () => {
    ...
    assert.ok(
      component.properties?.some(
        (p) => p.name === "internal:SrcFile" && p.value === "/some/path",
      ),
    );
  });
  ...
  it("should handle extension with no publisher", () => {
    ...
    const component = toComponent(extInfo);
    assert.ok(component);
    // cdx-purl requires a namespace (publisher) for vscode-extension purls.
    // Extensions without a publisher get a name-based bom-ref and no purl.
    assert.strictEqual(component["bom-ref"], "standalone-ext");
  });
