...
export function toGemPurl(name, version, platform) {
  ...
  return new PackageURL(
    ...
    name,
    version,
    gemPlatform && gemPlatform !== "ruby" ? { platform: gemPlatform } : null,
    null,
  ).toString();
}
...
export function getCompactIndexCacheDir() {
  return (
    ...
    join(
      process.env.BUNDLE_USER_CACHE || join(homedir(), ".bundle", "cache"),
      "compact_index",
    )
  );
}
...
export async function enrichGemsFromLocalCache(pkgList, options = {}) {
  ...
  for (const p of pkgList) {
    ...
    const platform = p.purl
      ? PackageURL.fromString(p.purl).qualifiers?.platform
      : undefined;
...
function gemComponentPlatform(p) {
  ...
  try {
    return PackageURL.fromString(p.purl).qualifiers?.platform || "ruby";
  } catch (_err) {
    return "ruby";
  }
}
...
export async function getRubyGemsMetadata(pkgList) {
  ...
  const RUBYGEMS_V1_VERSIONS_URL =
    process.env.RUBYGEMS_V1_VERSIONS_URL ||
    ...
    if (versionedPkgs.length > 1) {
      try {
        ...
        const res = await cdxgenAgent.get(
          `${RUBYGEMS_V1_VERSIONS_URL}${name}.json`,
          apiOptions,
        );
 ...
 */
async function enrichGemFromVersionEndpoint(p, v2Url, v1Url, apiOptions) {
  try {
    if (DEBUG_MODE) {
      console.log(`Querying rubygems.org for ${p.name}`);
    }
    // The v2 versions endpoint returns the `ruby` platform variant unless a
    // platform is requested explicitly, so pass along the platform qualifier
    // to retrieve the metadata for the native build that was resolved.
    const platform = gemComponentPlatform(p);
    const platformQuery =
    ...
    const fullUrl = p.version
      ? `${v2Url}${p.name}/versions/${simplifyRubyVersion(
          p.version,
        )}.json${platformQuery}`
      : `${v1Url}${p.name}.json`;
