 ...
 */
export function deriveSpecVersionOutputPath(inputPath, specVersion) {
  const suffix = `${specVersion}`.replace(/\./gu, "_");
  if (!inputPath) {
    return `bom-${suffix}.json`;
  }
  for (const extension of [".cdx.bin", ".cdx.json", ".spdx.json"]) {
