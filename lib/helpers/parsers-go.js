...
import { applyPurl, tryBuildPurl, tryParsePurl } from "./purl.js";
...
export async function getGoPkgComponent(group, name, version, hash) {
  ...
  // cdx-purl requires a namespace for golang purls. When the caller passes an
  // empty group (the common case — the full module path is in `name`), derive
  // the namespace by splitting on the last slash. This produces the identical
  // purl string but with the namespace/name separation the strict library
  // expects: pkg:golang/github.com/foo/bar  →  namespace "github.com/foo",
  // name "bar".
  ...
  // A single-segment module path (`go4.org`, `go.opencensus.io`) has no golang
  // namespace, and cdx-purl requires one, so those modules get no purl at all
  // and fall back to a `type:name:version` bom-ref via applyPurl.
  // Replacing %2F with / keeps the namespace readable as the spec intends.
  const purlString = purlNamespace
    ? tryBuildPurl({
        type: "golang",
...
export async function parseGoModData(goModData, gosumMap) {
  ...
  for (let l of pkgs) {
    ...
    if (l.startsWith("module ")) {
      ...
      // A single-segment module path (`go4.org`, `go.opencensus.io`) has no
      // golang namespace and cdx-purl rejects it, so there is simply no purl.
      applyPurl(
        parentComponent,
