...
describe("auditEnvironment - JVM Code Execution", () => {
  test("should skip JVM build-tool variables for explicit non-JVM project types", () => {
    const warnings = auditEnvironment(
      {
        JAVA_TOOL_OPTIONS: "-javaagent:/evil/agent.jar",
        MVN_ARGS: "-javaagent:/evil/maven-agent.jar",
        NODE_OPTIONS: "--require ./evil.js",
      },
      { projectType: ["go"] },
    );
    assert.ok(!warnings.some((w) => w.variable === "JAVA_TOOL_OPTIONS"));
    assert.ok(!warnings.some((w) => w.variable === "MVN_ARGS"));
    assert.ok(warnings.some((w) => w.variable === "NODE_OPTIONS"));
  });

  test("should audit JVM build-tool variables when a JVM project type is explicit", () => {
    const warnings = auditEnvironment(
      { JAVA_TOOL_OPTIONS: "-javaagent:/evil/agent.jar" },
      { projectType: "go,mvn" },
    );
...
describe("auditEnvironment - Credential Exposure", () => {
  ...
  test("should NOT flag DESKTOP_SESSION and other session-type vars (issue #4328)", () => {
    // A bare _SESSION suffix names a session/desktop type, not a credential
    const warnings = auditEnvironment({
    ...
    assert.ok(!warnings.some((w) => w.variable === "DESKTOP_SESSION"));
    assert.ok(!warnings.some((w) => w.variable === "GDM_SESSION"));
    assert.ok(!warnings.some((w) => w.variable === "GNOME_DESKTOP_SESSION_ID"));
    assert.ok(!warnings.some((w) => w.variable === "XDG_SESSION_TYPE"));
    assert.ok(!warnings.some((w) => w.type === "credential-exposure"));
  });

  test("should flag session identifier vars with _SESSION_ID / SESSIONID suffixes", () => {
    const warnings = auditEnvironment({
      MY_SESSION_ID: "abc123",
      APP_SESSIONID: "xyz789",
      "WEB_SESSION-ID": "sess_42",
    });
    assert.ok(warnings.some((w) => w.variable === "MY_SESSION_ID"));
    assert.ok(warnings.some((w) => w.variable === "APP_SESSIONID"));
    assert.ok(warnings.some((w) => w.variable === "WEB_SESSION-ID"));
  });

  test("should NOT flag OS/desktop session identifiers that match the naming pattern (issue #4328 follow-up)", () => {
    // Realistic values taken from the components that set each variable:
    // GNOME (deprecated marker), systemd logind (numeric id), KDE Konsole
    // (hex UUID), Apple Terminal/iTerm2 (window/tab/pane indices + UUID),
    // sudo/doas (invoking login name), Wayland launchers (focus token).
    const benignSessionEnv = {
      GNOME_DESKTOP_SESSION_ID: "this-is-deprecated",
      KDE_SESSION_ID: "12",
      SHELL_SESSION_ID: "865a093a1b6b4624b0e0c97c70bb55eb",
      TERM_SESSION_ID: "w0t0p0:BD2E7E7A-9BB7-4C1B-9C34-A3845F5C29C4",
      ITERM_SESSION_ID: "w0t0p0:6F1D2A88-CD4A-4C57-8D5A-1F2B3C4D5E6F",
      XDG_SESSION_ID: "2",
      SUDO_USER: "alice",
      DOAS_USER: "bob",
      XDG_ACTIVATION_TOKEN: "be6c1c5c-3f02-435c-84a7-b95c4e71a29f",
    };
    const warnings = auditEnvironment(benignSessionEnv);
    assert.deepStrictEqual(
      warnings.filter((w) => w.type === "credential-exposure"),
      [],
      `Unexpected credential-exposure findings: ${warnings
        .filter((w) => w.type === "credential-exposure")
        .map((w) => w.variable)
        .join(", ")}`,
    );
  });

  test("credential var exceptions are exact-match; lookalike suffixes are still flagged", () => {
    const warnings = auditEnvironment({
      APP_USER: "svc-identity",
      CI_TOKEN: "glpat-xyz",
    });
    assert.ok(warnings.some((w) => w.variable === "APP_USER"));
    assert.ok(warnings.some((w) => w.variable === "CI_TOKEN"));
  });

  test("should keep flagging SSH_USER_AUTH (only set by sshd with ExposeAuthInfo yes)", () => {
    const warnings = auditEnvironment({
      SSH_USER_AUTH: "/run/user/1000/ssh-auth-info",
    });
    assert.ok(warnings.some((w) => w.variable === "SSH_USER_AUTH"));
    assert.ok(warnings.some((w) => w.type === "credential-exposure"));
  });
