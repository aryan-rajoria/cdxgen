...
import { detectAiModelVariants } from "../../inventory/aiModelVariants.js";
import {
  formatHbomHardwareClassSummary,
  getHbomSummary,
  isHbomLikeBom,
} from "../../inventory/hbomAnalysis.js";
import {
  getContainerFileInventoryStats,
  getPropertyValue,
} from "../../inventory/inventoryStats.js";
import { getTrustedPublishingComponentCounts } from "../../inventory/provenanceUtils.js";
...
const isAiDatasetComponent = (component) =>
  component?.type === "data" ||
  getPropertyValue(component, "cdx:ai:kind") === "dataset" ||
  (component?.data || []).some((entry) => entry?.type === "dataset");

const datasetIdentity = (component) =>
  component?.group ? `${component.group}/${component.name}` : component?.name;

const referencedDatasetName = (dataset, bomJson) => {
  if (dataset?.name) {
    return dataset.name;
  }
  ...
  return datasetIdentity(resolved);
};

function formatShortList(values, max = 4) {
  const unique = uniqueValues(values);
  if (!unique.length) {
    return "";
  }
  return `${unique.slice(0, max).join(", ")}${
    unique.length > max ? ` and ${unique.length - max} more` : ""
  }`;
}

function getAiNarrativeSummary(bomJson) {
  const aiComponents = (bomJson?.components || []).filter((component) =>
    isAiComponent(component),
  );
  const aiServices = (bomJson?.services || []).filter(
    (service) =>
      getPropertyValue(service, "cdx:ai:kind") || service?.provider?.name,
  );
  if (!aiComponents.length && !aiServices.length) {
    return "";
  }
  const models = aiComponents.filter(
    (component) =>
      component?.type === "machine-learning-model" ||
      getPropertyValue(component, "cdx:ai:kind") === "model",
  );
  const standaloneDatasets = (bomJson?.components || [])
    .filter((component) => isAiDatasetComponent(component))
    .map((component) => datasetIdentity(component));
  const datasets = uniqueValues(
    standaloneDatasets.concat(
      models.flatMap(
        (component) =>
          component?.modelCard?.modelParameters?.datasets?.map((dataset) =>
            referencedDatasetName(dataset, bomJson),
          ) || [],
      ),
    ),
  );
  const datasetSizedCount = (bomJson?.components || []).filter(
    (component) =>
      isAiDatasetComponent(component) &&
      component?.data?.some((entry) => entry?.description),
  ).length;
  const tasks = uniqueValues(
    models.map(
      (component) =>
  ...
  const pedigreeModels = models.filter(
    (component) =>
  ...
  const usageProviders = uniqueValues(
    aiServices.map(
      (service) => service?.provider?.name || service?.group || service?.name,
    ),
  );
  const sentences = [];
  sentences.push(
    `It inventories ${models.length} AI model(s), ${aiServices.length} AI service(s), and ${datasets.length} dataset(s).`,
  );
  if (tasks.length) {
    sentences.push(`Documented model tasks include ${formatShortList(tasks)}.`);
  }
  if (datasets.length) {
    sentences.push(
      `Recorded training or evaluation datasets include ${formatShortList(datasets)}.`,
    );
  }
  if (pedigreeModels.length) {
    sentences.push(
      `Pedigree information is captured for ${pedigreeModels.length} model(s), which helps reviewers trace upstream bases and downstream variants.`,
    );
  }
  if (variants.length) {
    sentences.push(
      `Detected model variants include ${formatShortList(variants)}, so modified artifacts remain visible in the BOM.`,
    );
  }
  if (usageProviders.length) {
    sentences.push(
      `Usage references span ${usageProviders.length} provider or runtime endpoint(s), including ${formatShortList(usageProviders)}.`,
    );
  }
  if (datasetSizedCount) {
    sentences.push(
      `Dataset sizing metadata is captured for ${datasetSizedCount} dataset(s).`,
    );
  }
...
export function findBomType(bomJson) {
  ...
  const aiComponentCount = (bomJson?.components || []).filter((component) =>
    isAiComponent(component),
  ).length;
  const aiDatasetCount = (bomJson?.components || []).filter((component) =>
    isAiDatasetComponent(component),
  ).length;
  ...
  } else if (aiComponentCount > 0 && aiDatasetCount > 0) {
    bomType = "AI-BOM";
    description = "AI Bill-of-Materials (AI-BOM)";
  } else if (dataCount > 0) {
    bomType = "ML-BOM";
...
export function textualMetadata(bomJson) {
  ...
  const aiNarrativeSummary = getAiNarrativeSummary(bomJson);
  if (aiNarrativeSummary) {
    text = `${text} ${aiNarrativeSummary}`;
  }
  ...
  if (instrumentedCount > 0) {
    text = `${text} This SBOM contains ${instrumentedCount} dynamically instrumented library components discovered during process execution tracing. These libraries were loaded at runtime and mapped back to system packages.`;
  }
  // Summarize AI code-authorship provenance and human-oversight signals when the
  // cdx:ai:codegen:* / cdx:ai:oversight:* properties are present at the BOM root
  // (emitted by `-t ai-provenance` and cdx-audit).
  const aiProvenanceText = summarizeAiProvenance(bomJson);
  if (aiProvenanceText) {
    text = `${text} ${aiProvenanceText}`;
  }
 ...
 */
export function summarizeAiProvenance(bomJson) {
  const props = bomJson?.properties;
  if (!Array.isArray(props) || !props.length) {
    return "";
  }
  const get = (name) => props.find((p) => p?.name === name)?.value;
  if (get("cdx:ai:codegen:detected") !== "true") {
    return "";
  }
  let text =
    "AI/LLM/agent involvement was detected in this project's development";
  const tools = get("cdx:ai:codegen:tools");
  if (tools) {
    text = `${text} (${tools})`;
  }
  const band = get("cdx:ai:codegen:confidence:band");
  const confidence = get("cdx:ai:codegen:confidence");
  if (confidence) {
    text = `${text} with ${band ? `${band} ` : ""}confidence ${confidence}`;
  }
  text = `${text}.`;

  // Human-oversight rigor, when the oversight layer produced a score.
  const oversightScore = get("cdx:ai:oversight:score");
  const oversightBand = get("cdx:ai:oversight:band");
  if (oversightScore && oversightBand) {
    text = `${text} The human-oversight (process-rigor) score is ${oversightScore} (${oversightBand}).`;
    const details = [];
    const vdr = get("cdx:ai:oversight:verificationDebtRatio");
    if (vdr && vdr !== "unavailable") {
      details.push(`verification-debt ratio ${vdr}`);
    }
    const reviewCoverage = get("cdx:ai:oversight:reviewCoverage");
    if (reviewCoverage && reviewCoverage !== "unavailable") {
      details.push(`review coverage ${reviewCoverage}`);
    }
    const ciWeakening = get("cdx:ai:oversight:ciWeakeningEvents");
    if (ciWeakening && ciWeakening !== "0") {
      details.push(`${ciWeakening} CI-weakening event(s)`);
    }
    const aiLineRatio = get("cdx:ai:oversight:aiLineRatio");
    if (aiLineRatio) {
      details.push(`AI line ratio ${aiLineRatio}`);
    }
    const aiModels = get("cdx:ai:codegen:models");
    if (aiModels) {
      details.push(`AI models: ${aiModels}`);
    }
    const aiAgents = get("cdx:ai:codegen:agents");
    if (aiAgents) {
      details.push(`AI agents: ${aiAgents}`);
    }
    if (details.length) {
      text = `${text} Signals: ${joinArray(details)}.`;
    }
    const dataSources = get("cdx:ai:oversight:dataSources");
    if (dataSources) {
      text = `${text} Oversight data sources: ${dataSources}.`;
    }
  }
  return text;
}
