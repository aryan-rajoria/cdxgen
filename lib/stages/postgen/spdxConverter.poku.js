...
describe("convertCycloneDxToSpdx", () => {
  ...
  it("preserves AI services and machine-learning-model properties in SPDX export extensions", () => {
    const bom = sampleBom();
    bom.services = [
      {
    ...
    const aiServiceElement = spdxJson["@graph"].find(
      (element) =>
        getExtensionPropertyMap(element).get("bomRef") ===
        "urn:service:ai:openai:OpenAI-API",
    );
    const aiModelElement = spdxJson["@graph"].find(
      (element) =>
        getExtensionPropertyMap(element).get("bomRef") ===
        "urn:cdx:ai:model:openai:gpt-4o-mini",
    );

    assert.ok(aiServiceElement, "expected SPDX element for AI service");
    assert.ok(aiModelElement, "expected SPDX element for AI model component");
    assert.strictEqual(
      getExtensionPropertyMap(aiServiceElement).get(
        "properties.cdx:ai:modelId",
      ),
      "gpt-4o-mini",
    );
    assert.strictEqual(
      getExtensionPropertyMap(aiModelElement).get("properties.cdx:ai:provider"),
      "openai",
    );
  });
...
describe("CycloneDX-to-SPDX with introspection surface", () => {
  it("converts a BOM carrying introspection properties and annotations", () => {
    const bom = sampleBom();
    bom.metadata.properties.push(
      { name: "cdx:introspection:schemaVersion", value: "1.0" },
      { name: "cdx:introspection:tier", value: "manifest" },
      { name: "cdx:introspection:score", value: "55" },
      { name: "cdx:introspection:confidence", value: "medium" },
      { name: "cdx:introspection:ledgerComplete", value: "true" },
      { name: "cdx:introspection:ecosystem:dart:tier", value: "manifest" },
      { name: "cdx:introspection:ecosystem:dart:score", value: "55" },
      { name: "cdx:introspection:remediationCount", value: "1" },
    );
    bom.annotations = [
      {
        subjects: [bom.serialNumber],
        annotator: { component: { type: "application", name: "cdxgen" } },
        timestamp: "2026-08-30T00:00:00Z",
        text: "Build introspection: overall manifest (55/100), confidence medium; 1 remediation(s) ranked.",
      },
    ];
    const spdxJson = convertCycloneDxToSpdx(bom, { projectName: "demo-app" });
    assert.ok(spdxJson["@graph"], "the SPDX document is produced");
    assert.strictEqual(validateSpdx(spdxJson), true);
  });
});
