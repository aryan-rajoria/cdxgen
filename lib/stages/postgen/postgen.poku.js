...
it("postProcess does not move local Hugging Face AI inventory into formulation", () => {
  const bomNSData = {
    bomJson: {
      bomFormat: "CycloneDX",
      specVersion: "1.7",
...
it("postProcess strips 1.7-only cryptoProperties.algorithmProperties fields when downgrading to 1.6", () => {
...
it("postProcess preserves 1.7 cryptoProperties.algorithmProperties at spec version 1.7", () => {
  const bomNSData = {
    bomJson: {
      bomFormat: "CycloneDX",
      specVersion: "1.7",
...
it("postProcess preserves user-supplied patentAssertions at spec version 1.7 (round-trip)", () => {
...
it("postProcess strips root-level citations (a 1.7-only element) when downgrading to 1.6", () => {
...
it("a 1.7-generate-then-downgrade-to-1.6 run is root-equivalent to a direct 1.6 generation", () => {
...
it("postProcess removes service evidence for CycloneDX 1.7 output", () => {
  const bomNSData = {
    bomJson: {
      bomFormat: "CycloneDX",
      specVersion: "1.7",
...
it("postProcess strips license attributes unsupported by each spec version", () => {
...
it("postProcess produces a schema-valid bom for a custom npm license across spec versions", async () => {
  ...
  for (const specVersion of [1.6, 1.7]) {
    ...
    const result = postProcess(bomNSData, { specVersion });
...
it("postProcess does not merge MCP config inventory discovered during formulation", () => {
...
it("postProcess fails on prohibited licenses when failOnError is set", () => {
  ...
  assert.throws(
    () => postProcess(makeBom("GPL-3.0-only"), opts),
    /License policy violation/,
  );

  // A permissive license passes without error.
  const result = postProcess(makeBom("MIT"), opts);
