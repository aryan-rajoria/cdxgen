...
it("extractBuildOnlyComponents moves host-only components out of the assembly", () => {
  ...
  // Without a formulation section there is nowhere to move them to.
  const untouched = buildBom();
  assert.deepStrictEqual(
    extractBuildOnlyComponents(untouched, { specVersion: 1.6 }),
    [],
  );
  assert.deepStrictEqual(untouched.components.length, 3);
  const bomJson = buildBom();
  const moved = extractBuildOnlyComponents(bomJson, {
    includeFormulation: true,
    specVersion: 1.6,
  });
  assert.deepStrictEqual(
  ...
  // formulation is a 1.5+ section.
  const oldSpecBom = buildBom();
  assert.deepStrictEqual(
    extractBuildOnlyComponents(oldSpecBom, {
      includeFormulation: true,
      specVersion: 1.4,
    }),
    [],
