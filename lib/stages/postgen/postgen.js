...
import { addFormulationSection } from "../../ecosystems/formulationParsers.js";
import { getContainerFileInventoryStats } from "../../ecosystems/inventoryStats.js";
import { enhanceBom } from "../../ecosystems/licenseEnhancer.js";
import { buildReleaseNotesFromGit } from "../../ecosystems/source.js";
...
function applyLicenseEnhancement(bomJson, options) {
  const licenseEnhance =
    ...
    process.env.CDXGEN_LICENSE_ENHANCE !== "false";
  if (!licenseEnhance || !bomJson) {
    return bomJson;
  }
  const licensePolicy =
    options?.licensePolicy || process.env.CDXGEN_LICENSE_POLICY;
  enhanceBom(bomJson, {
    licenseEnrich:
      options?.licenseEnrich !== false &&
      process.env.CDXGEN_LICENSE_ENRICH !== "false",
    licensePolicy,
    licenseRef:
      options?.licenseRef || process.env.CDXGEN_LICENSE_REF === "true",
