...
describe("Group B — healthy projects must stay silent", () => {
  // fixture, -t, measured overall tier, the toolchain the tier needs.
  // Every row but go reaches its tier from a lock file alone. go-smoke ships
  // no go.sum, so `resolved` is a claim about a machine that has go: without
  // the toolchain the row measures the environment, not the feature, and its
  // remediation is the correct report rather than a false positive.
  const rows = [
    ["go-smoke", "go", "resolved", "go"],
  ...
  for (const [fixture, projectType, tier, requiredTool] of rows) {
    if (requiredTool && !toolAnswers(requiredTool)) {
      log(`${fixture}: ${requiredTool} is not installed; the row is skipped`);
      continue;
    }
...
describe("Group D — commands are shaped for the project's own wrappers and managers", () => {
  ...
  // The fixture ships both wrapper spellings. An action carries both
  // variants whatever the host is — `command` is shaped for posix and
  // `windows` for cmd — so the report's own fields are platform-invariant
  // and only the command the run *executed* follows the host.
  ...
  it("the maven repair names the wrapper the run itself executed", async () => {
    ...
    try {
      ...
      // One detection is recorded per action, and it names the posix
      // wrapper the posix command was shaped from.
      assert.equal(build.shapedBy, "wrapper:./mvnw");
      assert.equal(executableOf(build.command), "mvnw");
      ...
      // Only on posix do the two spellings meet: the catalog's build actions
      // carry no `windows` command, so the entry proposes the posix wrapper
      // on every host while the run itself executes mvnw.cmd on Windows.
      // Restore this cross-check on both platforms once the build actions
      // gain a Windows variant.
      if (!windows) {
