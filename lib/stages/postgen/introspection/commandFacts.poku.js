...
describe("collectCommandFacts()", () => {
  ...
  // Windows has no executable bit for chmod to clear, so the inexecutable
  // wrapper cannot be constructed there at all: the fixture would be an
  // ordinary readable file and the collector would be right to call it
  // executable. The forced posix platform only steers which spellings are
  // looked for; the permission itself comes from the real file system.
  const posixPermissions = process.platform !== "win32";
  (posixPermissions ? it : it.skip)("sees an inexecutable wrapper", () => {
    const dir = projectWith({
      gradlew: "#!/bin/sh\n",
      "gradlew:mode": 0o644,
    });
