...
function canonicalEcosystemFor(projectType) {
  ...
  return (
    ...
    (isVersionedJvmToolProjectType(normalizedType)
      ? ALIAS_TO_ECOSYSTEM.get(normalizedType.replace(/[\d.]+$/, ""))
      : undefined)
  );
}
...
function assessEcosystem(row, bomJson, facts) {
  ...
  const degradations = [];
  const seenDegradations = new Set();
  for (const event of row.events) {
    if (!REMEDIABLE_KINDS.has(event.kind)) {
      continue;
    }
    if (event.remediationId && corroborated && state !== "at-ceiling") {
      if (!seenDegradations.has(event.remediationId)) {
        seenDegradations.add(event.remediationId);
        degradations.push({
          remediationId: event.remediationId,
          impact: event.impact,
          detail: event.detail,
