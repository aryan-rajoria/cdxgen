...
describe("evaluateRule", () => {
  ...
  it("should detect an unversioned vendored native library (CARGO-SEC-001)", async () => {
    const rules = await loadRules(RULES_DIR);
    const rule = rules.find((r) => r.id === "CARGO-SEC-001");
    assert.ok(rule, "CARGO-SEC-001 rule should exist");
    ...
    const bom = makeBom([
      {
        ...
        properties: [
          { name: "cdx:cargo:linkage", value: "static" },
          { name: "cdx:cargo:nativeLibrary", value: "z" },
          { name: "cdx:cargo:providedBy", value: "pkg:cargo/libz-sys@1.1.29" },
        ],
      },
      // A versioned one is reported by CARGO-SEC-002 instead.
      {
        type: "library",
        name: "openssl",
        version: "3.6.3",
        purl: "pkg:generic/openssl@3.6.3",
        "bom-ref": "pkg:generic/openssl@3.6.3",
        properties: [{ name: "cdx:cargo:linkage", value: "static" }],
      },
    ]);

    const findings = await evaluateRule(rule, bom);
    assert.strictEqual(findings.length, 1);
    assert.strictEqual(findings[0].ruleId, "CARGO-SEC-001");
    assert.strictEqual(findings[0].severity, "medium");
  });

  it("should detect a crate from an alternate registry in formulation (CARGO-SEC-004)", async () => {
    const rules = await loadRules(RULES_DIR);
    const rule = rules.find((r) => r.id === "CARGO-SEC-004");
    assert.ok(rule, "CARGO-SEC-004 rule should exist");

    // A build dependency lives in formulation, so the rule has to look there
    // as well as in the assembly.
    const bom = makeBom(
      [],
      [],
      [
        makeComponent("internal-lib", "1.0.0", [
          ["cdx:cargo:alternateRegistry", "true"],
          ["cdx:cargo:sourceKind", "registry"],
          ["cdx:cargo:sourceUrl", "https://cargo.internal.example.com/index"],
        ]),
      ],
    );
    ...
    const findings = await evaluateRule(rule, bom);
    assert.strictEqual(findings.length, 1);
    assert.strictEqual(findings[0].ruleId, "CARGO-SEC-004");
  });

  it("should detect a proc-macro crate moved to formulation (CARGO-SEC-005)", async () => {
    const rules = await loadRules(RULES_DIR);
    const rule = rules.find((r) => r.id === "CARGO-SEC-005");
    assert.ok(rule, "CARGO-SEC-005 rule should exist");

    const bom = makeBom(
      [],
      [],
      [
        makeComponent("serde_derive", "1.0.229", [
          ["cdx:cargo:procMacro", "true"],
          ["cdx:cargo:hostOnly", "true"],
        ]),
      ],
    );
    ...
    const findings = await evaluateRule(rule, bom);
    assert.strictEqual(findings.length, 1);
    assert.strictEqual(findings[0].ruleId, "CARGO-SEC-005");
  });
