...
it("summarizes AI-BOM models, datasets, pedigree, variants, and usage", () => {
  const summary = textualMetadata({
    bomFormat: "CycloneDX",
    specVersion: "1.7",
    metadata: {
      timestamp: "2026-01-01T00:00:00Z",
      tools: { components: [{ name: "cdxgen" }] },
      component: { name: "ai-app", type: "application" },
    },
    components: [
      {
        ...
        modelCard: {
          modelParameters: {
            ...
            datasets: [{ name: "openbmb/UltraFeedback" }],
          },
        },
        pedigree: {
          ancestors: [{ group: "mistralai", name: "Mistral-7B-v0.1" }],
          notes:
  ...
  assert.match(
    summary,
    /It inventories 1 AI model\(s\), 1 AI service\(s\), and 1 dataset\(s\)\./u,
  );
  assert.match(summary, /Documented model tasks include text-generation\./u);
  assert.match(
    summary,
    /Recorded training or evaluation datasets include openbmb\/UltraFeedback\./u,
  );
  assert.match(summary, /Pedigree information is captured for 1 model\(s\)/u);
  assert.match(
    summary,
    /Detected model variants include distilled, abliterated, unlocked/u,
  );
  assert.match(
    summary,
    /Usage references span 1 provider or runtime endpoint\(s\), including ollama\./u,
  );
});

it("labels dataset-only AI inventories as AI-BOM and reports dataset sizing", () => {
  const summary = textualMetadata({
    bomFormat: "CycloneDX",
    specVersion: "1.7",
    metadata: {
      timestamp: "2026-01-01T00:00:00Z",
      tools: { components: [{ name: "cdxgen" }] },
      component: { name: "AppThreat", type: "application" },
    },
    components: [
      {
        "bom-ref": "https://huggingface.co/datasets/AppThreat/blint-db",
        type: "data",
        group: "AppThreat",
        name: "blint-db",
        scope: "excluded",
        ...
        data: [
          {
            type: "dataset",
            name: "AppThreat/blint-db",
            contents: {
              url: "https://huggingface.co/datasets/AppThreat/blint-db",
            },
            description: "Dataset size: 12,345 rows, 1 split(s)",
          },
        ],
      },
    ],
  ...
  assert.match(summary, /This AI Bill-of-Materials \(AI-BOM\) document/u);
  assert.match(
    summary,
    /It inventories 0 AI model\(s\), 0 AI service\(s\), and 1 dataset\(s\)\./u,
  );
  assert.match(
    summary,
    /Dataset sizing metadata is captured for 1 dataset\(s\)\./u,
  );
});

it("classifies mixed model-and-dataset AI inventories as AI-BOM", () => {
  assert.deepStrictEqual(
    findBomType({
      bomFormat: "CycloneDX",
      specVersion: "1.7",
      ...
      components: [
        {
          ...
          properties: [{ name: "cdx:ai:kind", value: "model" }],
          modelCard: {
            modelParameters: {
              task: "text-generation",
              datasets: [
                { ref: "https://huggingface.co/datasets/team/instructions" },
              ],
            },
          },
        },
        {
          "bom-ref": "https://huggingface.co/datasets/team/instructions",
          type: "data",
          group: "team",
          name: "instructions",
          properties: [{ name: "cdx:ai:kind", value: "dataset" }],
          data: [{ type: "dataset", name: "team/instructions" }],
        },
      ],
    }),
    {
      bomType: "AI-BOM",
      bomTypeDescription: "AI Bill-of-Materials (AI-BOM)",
    },
  );
});

it("summarizeAiProvenance from root properties", () => {
  // No properties / no AI signal -> empty string.
  assert.strictEqual(summarizeAiProvenance({}), "");
  assert.strictEqual(
    summarizeAiProvenance({
      properties: [{ name: "cdx:ai:codegen:detected", value: "false" }],
    }),
    "",
  );

  // Codegen-only provenance.
  const codegenOnly = summarizeAiProvenance({
    properties: [
      { name: "cdx:ai:codegen:detected", value: "true" },
      { name: "cdx:ai:codegen:tools", value: "claude-code,github-copilot" },
      { name: "cdx:ai:codegen:confidence", value: "0.99" },
      { name: "cdx:ai:codegen:confidence:band", value: "high" },
    ],
  });
  assert.ok(codegenOnly.includes("AI/LLM/agent involvement"));
  assert.ok(codegenOnly.includes("claude-code,github-copilot"));
  assert.ok(codegenOnly.includes("high confidence 0.99"));

  // Codegen + oversight, with git-only unavailable metrics filtered out.
  const withOversight = summarizeAiProvenance({
    properties: [
      { name: "cdx:ai:codegen:detected", value: "true" },
      ...
      { name: "cdx:ai:oversight:verificationDebtRatio", value: "unavailable" },
      { name: "cdx:ai:oversight:reviewCoverage", value: "unavailable" },
      { name: "cdx:ai:oversight:ciWeakeningEvents", value: "2" },
      { name: "cdx:ai:oversight:aiLineRatio", value: "0.4211" },
      { name: "cdx:ai:oversight:dataSources", value: "git,git-ai-notes" },
    ],
  });
  assert.ok(withOversight.includes("human-oversight"));
  assert.ok(withOversight.includes("0.4500 (weak)"));
  assert.ok(withOversight.includes("2 CI-weakening event(s)"));
  assert.ok(withOversight.includes("AI line ratio 0.4211"));
  assert.ok(withOversight.includes("git,git-ai-notes"));
  // Unavailable review metrics must not leak into the annotation.
  assert.ok(!withOversight.includes("unavailable"));
});
