  ...
  for (const f of cargoFiles) {
    ...
    if (
      ...
      if (cargoInstallResult.status !== 0 || cargoInstallResult.error) {
        ...
        if (
          !classifySpawnRestriction(CARGO_CMD) &&
          cargoInstallResult.error &&
...
export function createHaskellBom(path, options) {
  ...
  recordDegradation("haskell.freeze-missing", {
    ...
    impact: "components",
    detail:
      "No cabal.project.freeze was found, and the freeze file is the source the parser reads for the package set.",
  });
  return {};
}
