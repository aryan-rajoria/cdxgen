...
/**
 * Shallow-copies a nested component tree level by level so conversions run on
 * the copy instead of the caller's context objects.
 *
 * @param {object[]|undefined} components Nested component list
 * @returns {object[]|undefined} Copied list structure
 */
function cloneNestedComponents(components) {
  if (!Array.isArray(components)) {
    return components;
  }
  return components.map((comp) =>
    comp && typeof comp === "object"
      ? { ...comp, components: cloneNestedComponents(comp.components) }
      : comp,
  );
}
...
function anchorDetectedParent(
  ...
  if (
    ...
    // (issue #4320). The copy — nested workspace members included (issue
    // #4326) — keeps the caller's context object untouched.
    components.push(
      cleanParentComponent({
        ...detectedParent,
        components: cloneNestedComponents(detectedParent.components),
      }),
    );
  }
...
/**
 * Convert the transient keys parsers attach to package objects (`license`,
 * `homepage`, `repository`, `bugs`, `distribution`, `_integrity`,
 * `_shasum`, `qualifiers`) into their schema-valid CycloneDX equivalents
 * (`licenses`, `externalReferences`, `hashes`), or drop them when there is
 * no equivalent.
 *
 * Shared by the parent-component and nested-component cleaners so both levels
 * apply the exact same conversion. The hash conversion is the one
 * `processHashes` performs for flat components — nested and parent
 * components bypass `addComponent`, so it is replayed here.
 *
 * @param {object} comp Component to clean in place
 */
const convertTransientPackageKeys = (comp) => {
  if (comp._integrity || comp._shasum) {
    // Converted through a fresh carrier: `processHashes` mutates
    // `component.hashes`, and passing `comp` itself would iterate and append
    // to the same array when `comp.hashes` already exists.
    const hashCarrier = { hashes: [] };
    processHashes(
      { _integrity: comp._integrity, _shasum: comp._shasum },
      hashCarrier,
    );
    // `processHashes` deletes an empty `hashes` array, hence the Array.isArray
    // guard: an untrusted digest is dropped entirely (see normalizeHashContent).
    if (Array.isArray(hashCarrier.hashes) && hashCarrier.hashes.length) {
      const existingHashes = Array.isArray(comp.hashes) ? comp.hashes : [];
      const known = new Set(
        existingHashes.map((hash) => `${hash.alg}:${hash.content}`),
      );
      comp.hashes = existingHashes.concat(
        hashCarrier.hashes.filter(
          (hash) => !known.has(`${hash.alg}:${hash.content}`),
        ),
      );
    }
  }
  // The deletes are unconditional: parsers can leave explicitly-undefined
  // keys (`license: undefined`), which Ajv still counts as an additional
  ...
  delete comp.license;
  if (comp.homepage || comp.repository || comp.bugs || comp.distribution) {
...
export const cleanParentComponent = (comp) => {
  ...
  convertTransientPackageKeys(comp);
  cleanNestedComponents(comp.components);
  return comp;
};
 ...
 */
export const cleanNestedComponents = (components) => {
  if (!Array.isArray(components)) {
    return components;
  }
...
export function addComponent(
  ...
  if (!isRootPkg) {
    ...
    if (pkg.components) {
      // Nested members may still carry transient parser keys (homepage,
      // repository, license) that the Component schema rejects (issue #4326).
      component.components = cleanNestedComponents(pkg.components);
    }
