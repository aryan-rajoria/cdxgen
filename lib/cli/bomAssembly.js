...
export function addComponent(
  ...
  if (!isRootPkg) {
    ...
    if (options.specVersion >= 1.7) {
      // An external component is provided by the environment rather than
      // bundled with the product. The default is false, so the field is only
      // worth emitting when it is true.
