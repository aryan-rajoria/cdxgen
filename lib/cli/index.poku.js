...
describe("CLI tests", () => {
  ...
  describe("submitBom()", () => {
    ...
    it("submits a large BOM that would exceed the base64 JSON limit", async () => {
      const server = await startSubmitBomTestServer(async () => ({
        body: { success: true },
      }));
      const bomContent = {
        bomFormat: "CycloneDX",
        components: Array.from({ length: 5000 }, (_unused, index) => ({
      ...
      try {
        ...
        assert.equal(server.requests.length, 1);
        const parts = submittedParts(server.requests[0]);
        assert.deepEqual(JSON.parse(parts.bom.value), bomContent);
      } finally {
        await server.close();
      }
    });
