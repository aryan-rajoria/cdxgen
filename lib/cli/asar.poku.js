...
import { readAsarArchiveHeaderSync } from "../ecosystems/asarutils.js";
  ...
  describe("createAsarBom()", () => {
    it("catalogs ASAR archives, extracts nested npm metadata, and surfaces audit findings", async () => {
      ...
      try {
        ...
        assert.ok(
          String(getProp(sketchyAddon, "internal:SrcFile") || "").includes(
            `${archivePath}#/`,
          ),
        );

        const postProcessed = await postProcess(bomData, {
          bomAudit: true,
      ...
      try {
        ...
        const nestedArchiveComponents = (
          ...
          (component) =>
            ...
            String(getProp(component, "internal:SrcFile") || "").startsWith(
    ...
    it("produces a schema-valid BOM with ASAR signing crypto components", async () => {
      ...
      try {
        ...
        assert.strictEqual(await validateBom(postProcessed.bomJson), true);
        assert.ok(
    ...
    it("handles invalid ASAR archives gracefully", async () => {
      const fixtureRoot = mkdtempSync(join(tmpdir(), "cdxgen-asar-invalid-"));
      const bogusArchivePath = join(fixtureRoot, "bogus.asar");
      const validArchivePath = join(fixtureRoot, "valid.asar");
      writeFileSync(
        bogusArchivePath,
        "not a real asar archive, just text header data",
      );
      createAsarFixture(validArchivePath);
      try {
        const bomData = await createAsarBom(fixtureRoot, {
          installDeps: false,
          multiProject: true,
        ...
        assert.ok(bomData?.bomJson?.components?.length);
        const hasValidComponent = bomData.bomJson.components.some(
          (c) =>
            getProp(c, "cdx:file:kind") === "asar-archive" &&
            String(getProp(c, "internal:SrcFile")).endsWith("valid.asar"),
        );
        assert.ok(hasValidComponent, "expected valid.asar to be cataloged");
        ...
        try {
          await createAsarBom(fixtureRoot, {
            installDeps: false,
            multiProject: true,
            projectType: ["asar"],
            specVersion: 1.7,
    ...
    it("respects excludeType for asar", async () => {
      const fixtureRoot = mkdtempSync(join(tmpdir(), "cdxgen-asar-exclude-"));
      const archivePath = join(fixtureRoot, "app.asar");
      createAsarFixture(archivePath);
      try {
        const bomData = await createAsarBom(archivePath, {
        ...
        assert.equal(bomData.components, undefined);
      } finally {
        rmSync(fixtureRoot, { force: true, recursive: true });
      }
    });
