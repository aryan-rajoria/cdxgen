...
import { parseTerraformLockFile } from "../ecosystems/parsers-terraform.js";
 ...
 * @returns {Object} BOM object
 */
export function createTerraformBom(path, options) {
  let pkgList = [];
  const lockFiles = getAllFiles(
    path,
  ...
  for (const lockFile of lockFiles) {
    ...
    const result = parseTerraformLockFile(lockFile);
    if (result.pkgList?.length) {
      pkgList = pkgList.concat(result.pkgList);
    }
  }
