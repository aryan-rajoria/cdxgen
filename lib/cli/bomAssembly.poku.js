...
describe("bomAssembly", () => {
  ...
  describe("parent component overrides", () => {
    ...
    // same transient parser keys as the parent itself. Cleaned only at the top
    // level, the nested components fail the strict Component schema
    // (`additionalProperties: false` on `homepage`/`repository`/`license`).
    ...
    it("cleans transient keys from nested workspace member components", async () => {
      const { bomJson } = buildBomNSData(
        overrideOptions,
        [child],
        "npm",
    ...
    it("cleans nested member components retained under regular components", async () => {
      const pkgWithNestedMembers = {
        name: "app",
        version: "1.0.0",
      ...
      const { bomJson } = buildBomNSData(
        { specVersion: 1.6 },
        [pkgWithNestedMembers, child],
        "npm",
        {},
      );
      const appComponent = bomJson.components.find(
        (comp) => comp["bom-ref"] === "pkg:npm/app@1.0.0",
      );
      const nested = appComponent?.components?.[0];
      assert.ok(nested, "expected nested member to be retained");
      assert.strictEqual(nested.homepage, undefined);
      assert.strictEqual(nested.repository, undefined);
      assert.deepStrictEqual(
        (nested.externalReferences || []).map((ref) => ref.type).sort(),
        ["vcs", "website"],
      );
