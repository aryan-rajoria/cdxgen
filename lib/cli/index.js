...
import { parseBuildZigZon } from "../ecosystems/parsers-zig.js";
...
export async function createZigBom(path, options) {
  ...
  for (const zonFile of zonFiles) {
    ...
    const { pkgList: zonPkgs, parentComponent: zonParent } =
      parseBuildZigZon(zonFile);
    if (zonPkgs?.length) {
      pkgList = pkgList.concat(zonPkgs);
    }
...
export async function createMultiXBom(pathList, options) {
  ...
  if (pathScanConcurrency(pathList) > 1) {
    ...
    for (const path of pathList) {
      pathResults.push(await scanOnePath(path));
    }
  }
