    ...
    for (const f of filesToRestore) {
      ...
      if (DEBUG_MODE && (result.status !== 0 || result.error)) {
        ...
        if (result?.stderr?.includes("To install missing framework")) {
          ...
          console.log(
            "TIP: Try using the custom `ghcr.io/cdxgen/cdxgen-debian-dotnet6:v13` or `ghcr.io/cdxgen/cdxgen-debian-dotnet8:v13` container images.",
          );
        } else if (
          ...
          if (readEnvironmentVariable("CDXGEN_IN_CONTAINER") !== "true") {
            console.log(
              "Alternatively, try using the custom `ghcr.io/cdxgen/cdxgen-debian-dotnet6:v13` container image, which bundles nuget (mono) and a range of dotnet SDKs.",
            );
          }
        }
        console.log("---------");
