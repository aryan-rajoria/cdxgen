...
export async function submitBom(args, bomContents) {
  ...
  try {
    ...
    if (error.response && error.response.statusCode === 401) {
      ...
      console.log(
        `The server at ${serverUrl} does not accept a multipart/form-data POST. Check that the URL points to a Dependency-Track 4.x or later API server.`,
      );
    } else if (DEBUG_MODE) {
      console.log(
