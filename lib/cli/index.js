...
} from "../ecosystems/aiInventory.js";
import {
  detectMcpInventory,
  detectPythonMcpInventory,
  findJSImportsExports,
} from "../ecosystems/analyzer.js";
import {
  cleanupAsarTempDir,
...
import { expandBomAuditCategories } from "../ecosystems/auditCategories.js";
import {
...
import {
  ...
  readDosaiJsonFile,
} from "../ecosystems/dosai.js";
import { buildDynamicComponents } from "../ecosystems/dynamic.js";
import {
  extractJarArchive,
  getCratesMetadata,
  getMvnMetadata,
...
import { GIT_COMMAND } from "../ecosystems/envcontext.js";
import {
  ...
  extractToolRefs,
  getCppModules,
} from "../ecosystems/evidenceUtils.js";
import {
...
import {
  ...
  hasHbomProjectType,
} from "../ecosystems/hbom.js";
import { mergeHostInventoryBoms } from "../ecosystems/hostTopology.js";
import { enrichComponentWithMcpMetadata } from "../ecosystems/mcp.js";
import {
  createNpmWorkspacePurl,
  isPkgTypeOnlyImport,
} from "../ecosystems/npmutils.js";
import {
...
import { getLicenses } from "../ecosystems/spdx.js";
import { table } from "../ecosystems/table.js";
import {
          ...
          if (result.status !== 0 || result.error) {
            ...
            if (pomFiles.length === 1 || DEBUG_MODE || PREFER_MAVEN_DEPS_TREE) {
              ...
              if (
                ...
                console.log(
                  "1. Java version requirement: cdxgen container image bundles Java 24 with maven 3.9 which might be incompatible. Try running cdxgen with the custom JDK11-based image `ghcr.io/cyclonedx/cdxgen-java11:v13`.",
                );
              }
              console.log(
...
export async function createCsharpBom(path, options) {
    ...
    for (const f of filesToRestore) {
      ...
      if (DEBUG_MODE && (result.status !== 0 || result.error)) {
        ...
        if (result?.stderr?.includes("To install missing framework")) {
          ...
          console.log(
            "TIP: Try using the custom `ghcr.io/cyclonedx/cdxgen-debian-dotnet6:v13` or `ghcr.io/cyclonedx/cdxgen-debian-dotnet8:v13` container images.",
          );
        } else if (
          ...
          if (process.env?.CDXGEN_IN_CONTAINER !== "true") {
            console.log(
              "Alternatively, try using the custom `ghcr.io/cyclonedx/cdxgen-debian-dotnet6:v13` container image, which bundles nuget (mono) and a range of dotnet SDKs.",
            );
          }
        }
        console.log("---------");
  ...
  if (projAssetsFiles.length) {
    ...
    if (!pkgList.length || dependencies.length < 2) {
      ...
      console.log(
        "Alternatively, try using the custom `ghcr.io/cyclonedx/cdxgen-dotnet:v13` container image, which bundles a range of dotnet SDKs.",
      );
      options.failOnError && process.exit(1);
    }
  }
