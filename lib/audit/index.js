...
import { PROJECT_CONFIG_FILENAMES } from "../core/projectConfig.js";
import { fetchPomXmlAsJson } from "../ecosystems/ecosystems.js";
import { DEFAULT_HBOM_AUDIT_CATEGORIES } from "../inventory/auditCategories.js";
import {
...
import { getBomWithOras } from "../managers/oci.js";
import {
  auditBom,
  isHbomLikeBom,
  isObomLikeBom,
...
import {
  ...
  reflectOnRun,
} from "../stages/postgen/introspection/reflect.js";
import { buildIntrospectionJson } from "../stages/postgen/introspection/report.js";
import { scoreReflection } from "../stages/postgen/introspection/score.js";
...
export async function runDirectBomAuditFromBoms(inputBoms, options = {}) {
  ...
  for (const inputBom of inputBoms) {
    ...
    const findings = await auditBom(inputBom.bomJson, {
      ...directAuditOptions,
      getBomWithOras,
    });
    const result = {
    ...
    if (isIntrospectionEnabled(options)) {
      // The audited BOM was not generated by this run, so there is no ledger
      // of its own; the verdict rests on BOM structure alone and says so.
      try {
        const reflection = await reflectOnRun(inputBom.bomJson, options, {
          ledgerEvents: [],
          ledgerSource: "none",
          projectPath: inputBom.source || "",
        });
        reflection.scoring = scoreReflection(
          reflection,
        ...
        reflection.bom = {
          serialNumber: inputBom.bomJson?.serialNumber,
          componentCount: Array.isArray(inputBom.bomJson?.components)
            ? inputBom.bomJson.components.length
            : 0,
        };
        result.introspection = buildIntrospectionJson(
          reflection,
          reflection.scoring,
          options,
        );
 ...
 * @throws {Error} when gitRef is not a safe ref name
 * @returns {void}
 */
function cloneRepositoryToDir(repoUrl, cloneDir, gitRef) {
  ...
  if (gitRef) {
    if (!isSafeGitRefName(gitRef)) {
      const error = new Error("git ref contains unsupported characters");
      error.errorType = "clone";
...
/**
 * Remove cdxgen config files from a directory the audit is about to scan.
 *
 * Audit mode clones repositories cdxgen picked itself, from registry metadata,
 * so nobody approved the contents. A config file in such a checkout would let
 * the audited repository choose its own scan options, which is the audited
 * repository deciding how it gets audited.
 *
 * @param {string} dir directory to clear
 * @returns {void}
 */
function rejectProjectConfigFiles(dir) {
  if (!dir) {
    return;
  }
  for (const configFileName of PROJECT_CONFIG_FILENAMES) {
    ...
    console.warn(
      `Ignoring the config file '${configFileName}' found in the cloned repository ${dir}.`,
    );
    recordActivity({
      kind: "inspect",
      reason:
        "Removed a cdxgen config file from an audit checkout. Cloned repositories do not configure their own audit.",
      status: "completed",
      target: configPath,
    });
  }
...
export function resolveTargetSourceDirectory(cloneDir, target, resolution) {
  const selection = selectTargetSourceDirectory(cloneDir, target, resolution);
  rejectProjectConfigFiles(cloneDir);
  if (selection.scanDir !== cloneDir) {
    rejectProjectConfigFiles(selection.scanDir);
  }
  return selection;
}

/**
 * Pick the scan directory for a cloned target repository.
 *
 * @param {string} cloneDir cloned repository root
 * @param {object} target audit target
 * @param {object} resolution repository resolution metadata
 * @returns {{ confidence: string, scanDir: string }} selected directory and confidence
 */
function selectTargetSourceDirectory(cloneDir, target, resolution) {
  if (target.type === "npm") {
...
function buildChildOptions(options, target) {
  ...
  return {
    ...
    projectType,
    specVersion: DEFAULT_CDX_SPEC_VERSION,
  };
}
...
async function recheckFindingsAgainstDefaultBranch(
  ...
  try {
    ...
    const childOptions = {
      ...buildChildOptions(options, target),
      executeOsQuery,
    };
    const bomNSData =
...
export async function auditTarget(target, options) {
  ...
  try {
    ...
    // Evaluate the license policy against this dependency's child SBOM so
    // source-derived (and deeper transitive) licenses are covered, not just the
    // licenses declared in the parent BOM.
    const licenseViolations = options._licensePolicy
      ? collectPolicyViolations(processedBomJson, options._licensePolicy, {
...
export async function runAuditFromBoms(inputBoms, options) {
  ...
  if (extractedTargets.targets.length) {
    ...
    // Every target's repository lookup is known here, before the first clone,
    // so they are resolved in one round rather than one per iteration below.
    await prefetchGitUrlSources(
      extractedTargets.targets.map((target) => target.purl),
    );
  }
...
export async function runAudit(options) {
  ...
  const licensePolicy = options.licensePolicy
    ? loadPolicy(options.licensePolicy)
    : null;
  try {
    const auditOptions = {
      ...options,
      workspaceDir: workspaceContext.workspaceDir,
      // Preloaded so each target's child SBOM can be evaluated in auditTarget
      // without re-reading the policy file per dependency.
      _licensePolicy: licensePolicy,
    };
    const report = options.directBomAudit
      ? await runDirectBomAuditFromBoms(inputBoms, auditOptions)
      : await runAuditFromBoms(inputBoms, auditOptions);
    attachLicenseViolations(report, inputBoms, licensePolicy);
    return report;
  } finally {
 ...
 * @returns {void}
 */
export function attachLicenseViolations(report, inputBoms, policy) {
  if (!report || !policy) {
    return;
  }
  const seen = new Set();
  const violations = [];
  const add = (violation, source) => {
    const key = `${violation.ref}|${violation.license}|${violation.alert}`;
    if (seen.has(key)) {
      return;
    }
    seen.add(key);
    violations.push({ ...violation, source });
  };
  // Direct violations from the input BOM(s).
  for (const inputBom of inputBoms) {
    for (const violation of collectPolicyViolations(inputBom.bomJson, policy, {
      includeWarnings: true,
    })) {
      add(violation, inputBom.source);
    }
  }
  // Violations surfaced from each dependency's child SBOM (predictive mode).
  for (const result of report.results || []) {
    for (const violation of result.licenseViolations || []) {
      add(violation, formatTargetLabel(result.target) || result.repoUrl);
    }
  }
  report.licenseViolations = violations;
  report.summary = report.summary || {};
  report.summary.licenseViolations = violations.length;
  report.summary.licenseViolationsByAlert = violations.reduce(
    (counts, violation) => {
      counts[violation.alert] = (counts[violation.alert] || 0) + 1;
      return counts;
    },
    {},
  );
...
export function finalizeAuditReport(report, options) {
  ...
  // Prohibited (error-level) license violations are a hard fail regardless of
  // the supply-chain severity threshold.
  const licensePolicyFail = (report?.licenseViolations || []).some(
    (violation) => violation.alert === "error",
  );
  if (report?.auditMode === "direct") {
    ...
    return {
      exitCode: shouldFail || licensePolicyFail ? 3 : 0,
      output,
    };
  }
