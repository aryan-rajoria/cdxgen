...
import { hasTrustedPublishingProperties } from "../inventory/provenanceUtils.js";
...
function buildRegistryMetadataCandidate(componentPurl, component) {
  let purlObj;
  try {
    purlObj = Purl.parse(componentPurl);
  } catch {
    return undefined;
  }
...
export async function enrichInputBomsWithRegistryMetadata(inputBoms) {
  ...
  try {
    const enrichedCandidates = [
      // The audit scores publisher drift, ownership and release cadence, none
      // of which the local Cargo registry records: it knows when a version was
      // published but not by whom. This path pays the registry's rate limit
      // deliberately rather than audit against metadata that cannot answer the
      // questions the rules ask.
      ...(cargoCandidates.length
        ? await getCratesMetadata(cargoCandidates, { preferLocalCache: false })
        : []),
