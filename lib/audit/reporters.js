...
/**
 * Render the license policy violations as a standalone table. Returns an empty
 * string when there are no violations to show.
 *
 * @param {object[]} violations license violations
 * @returns {string} rendered table block (with heading) or empty string
 */
function renderLicenseViolationsTable(violations) {
  if (!violations?.length) {
    return "";
  }
  const alertRank = { error: 3, warning: 2, pass: 1 };
  const sorted = [...violations].sort(
    (a, b) => (alertRank[b.alert] || 0) - (alertRank[a.alert] || 0),
  );
  const rows = [["Alert", "Component", "License", "Category"]];
  for (const violation of sorted) {
    const component = violation.version
      ? `${violation.ref} @ ${violation.version}`
      : violation.ref;
    rows.push([
      (violation.alert || "").toUpperCase(),
      component,
      violation.license || "",
      violation.category || "Unstated License",
    ]);
  }
  const errors = sorted.filter((v) => v.alert === "error").length;
  const warnings = sorted.filter((v) => v.alert === "warning").length;
  const lines = [
    "License policy violations:",
    `Prohibited: ${errors}  Warnings: ${warnings}`,
    "",
    table(rows, {
      columns: [{ width: 9 }, { width: 50 }, { width: 32 }, { width: 20 }],
...
export function renderSarifReport(report, options = {}) {
  ...
  const toolVersion = report?.tool?.version || "v13";
  const log = {
    $schema: SARIF_SCHEMA,
...
export function renderDirectBomConsoleReport(report, options = {}) {
  const minSeverity = options.minSeverity || "low";
  const licenseBlock = renderLicenseViolationsTable(report?.licenseViolations);
  const licenseSuffix = licenseBlock ? `\n${licenseBlock}\n` : "";
