...
describe("auditTarget() default branch recheck", () => {
  it("downgrades severity when findings are fixed in default branch", async () => {
    ...
    const { auditTarget } = await esmock("./index.js", {
      ...
      "../core/activity.js": { isDryRun: false },
      "../stages/postgen/auditBom.js": { auditBom: auditBomStub },
      "../stages/postgen/postgen.js": {
        postProcess: sinon.stub().callsFake((bomNSData) => bomNSData),
      },
  ...
  it("logs non-Error default-branch recheck failures safely", async () => {
    ...
    const { auditTarget } = await esmock("./index.js", {
      ...
      "../core/logger.js": { thoughtLog: thoughtLogStub },
      "../inventory/source.js": {
        cleanupSourceDir: sinon.stub(),
        findGitRefForPurlVersion: sinon.stub().returns("1.0.0"),
        hardenedGitCommand: sinon.stub().returns({ status: 0 }),
...
describe("attachLicenseViolations", () => {
  const policy = {
    license_policies: [
  ...
  it("aggregates and dedupes input BOM and child SBOM violations", () => {
    const report = {
      summary: {},
      results: [
        {
          target: { purl: "pkg:npm/dep-a@1.0.0" },
          // Surfaced from the cloned child SBOM; one entry duplicates the input
          // BOM violation and must be collapsed.
          licenseViolations: [
            {
              ref: "pkg:npm/gpllib@1.0.0",
              license: "GPL-3.0-only",
              alert: "error",
              category: "Copyleft",
            },
            {
              ref: "pkg:npm/deeptransitive@9.9.9",
              license: "GPL-3.0-only",
              alert: "error",
              category: "Copyleft",
            },
    ...
    attachLicenseViolations(report, inputBoms, policy);

    // gpllib (shared) counted once; bsdlib (input only); deeptransitive (child only)
    assert.strictEqual(report.licenseViolations.length, 3);
    assert.strictEqual(report.summary.licenseViolations, 3);
    assert.deepStrictEqual(report.summary.licenseViolationsByAlert, {
      error: 2,
      warning: 1,
    });
    const deep = report.licenseViolations.find(
      (v) => v.ref === "pkg:npm/deeptransitive@9.9.9",
    );
    assert.ok(deep, "child-only transitive violation should be present");
  });

  it("is a no-op without a policy", () => {
    const report = { summary: {}, results: [] };
    attachLicenseViolations(report, [], null);
    assert.strictEqual(report.licenseViolations, undefined);
  });
});

describe("resolveTargetSourceDirectory", () => {
  it("removes cdxgen config files from an audit checkout", () => {
    const cloneDir = mkdtempSync(path.join(os.tmpdir(), "audit-clone-config-"));
    try {
      for (const configFileName of [
        ...
        writeFileSync(
          path.join(cloneDir, configFileName),
          '{"output":"/tmp/escaped.json"}',
        );
      }
      const selection = resolveTargetSourceDirectory(
        cloneDir,
        { type: "generic" },
        {},
      );
      assert.strictEqual(selection.scanDir, cloneDir);
      for (const configFileName of [
        ".cdxgenrc",
        ".cdxgen.json",
        ".cdxgen.yml",
        ...
        assert.ok(
          !existsSync(path.join(cloneDir, configFileName)),
          `${configFileName} should not survive in an audit checkout`,
        );
      }
    } finally {
  ...
  it("removes config files from a resolved package subdirectory too", () => {
    const cloneDir = mkdtempSync(path.join(os.tmpdir(), "audit-clone-mono-"));
    try {
      const packageDir = path.join(cloneDir, "packages", "left-pad");
      mkdirSync(packageDir, { recursive: true });
      writeJson(path.join(packageDir, "package.json"), {
        name: "left-pad",
        version: "1.0.0",
      });
      writeFileSync(path.join(packageDir, ".cdxgen.json"), "{}");
      writeFileSync(path.join(cloneDir, ".cdxgen.json"), "{}");
      const selection = resolveTargetSourceDirectory(
        cloneDir,
        { type: "npm" },
        { type: "npm", name: "left-pad" },
      );
      assert.strictEqual(selection.scanDir, packageDir);
      assert.ok(!existsSync(path.join(packageDir, ".cdxgen.json")));
      assert.ok(!existsSync(path.join(cloneDir, ".cdxgen.json")));
    } finally {
      rmSync(cloneDir, { force: true, recursive: true });
    }
  });
