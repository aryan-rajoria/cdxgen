...
describe("analyzeProject() dotnet crypto", () => {
  it("skips the dosai crypto rerun when the input BOM already carries crypto components", async () => {
    const tmpDir = mkdtempSync(join(tmpdir(), "cdxgen-evinse-crypto-"));
    try {
      const bomFile = join(tmpDir, "bom.cdx.json");
      writeBomFile(bomFile, [nugetComponent, cryptoComponent]);
  ...
  it("collects dosai crypto components when the input BOM has none", async () => {
