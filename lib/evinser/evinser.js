...
export async function createEvinseFile(sliceArtefacts, options) {
  ...
  // Analyzer metadata describes one run. When the input BOM already carries
  // facts from an earlier run (cdxgen --profile research, then evinse), the
  // fresh values replace them rather than sitting beside them.
  const metadataComponent = bomJson.metadata?.component;
  if (metadataComponent?.properties?.length && metadataProperties.length) {
    const refreshedNames = new Set(metadataProperties.map((p) => p.name));
    metadataComponent.properties = metadataComponent.properties.filter(
      (p) => !refreshedNames.has(p.name),
    );
  }
  appendComponentProperties(metadataComponent, metadataProperties);
