...
export async function analyzeProject(dbObjMap, options) {
  ...
  if (isDosaiDotnetLanguage(language)) {
    ...
    if (
      ...
      usageSlice = JSON.parse(fs.readFileSync(options.depsSlicesFile, "utf-8"));
    ...
    if (options.includeCrypto) {
      // When the input BOM was created in the same run, createCryptoCertsBom
      // already ran dosai crypto and its components are in the BOM, which
      // createEvinseFile enriches without deduping. Rerunning dosai crypto
      // here doubled both the analysis and every component in the final BOM.
      if (components.some((comp) => comp.type === "cryptographic-asset")) {
        if (DEBUG_MODE) {
          console.log(
            "Skipping the dosai crypto analysis since the input SBOM already contains cryptographic-asset components.",
          );
        }
      } else {
