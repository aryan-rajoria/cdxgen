...
function collectVendorFiles() {
  return listJsFiles(VENDOR_ROOT).map((f) => ({
    abs: f,
    // The allowlist and the manifest both key on posix-style paths, so the
    // separator is normalised here rather than at every comparison site.
    rel: path.relative(VENDOR_ROOT, f).split(path.sep).join("/"),
