...
// Whether assigning to `__proto__` mutates an object's prototype. True on
// Node.js, where the Object.prototype accessor is live. False under Deno, where
// the assignment instead defines an ordinary own property of that name. Probed
// rather than inferred from the runtime name so a change on either side is
// picked up automatically.
const PROTO_SETTER_MUTATES = (() => {
  const probe = {};
  // biome-ignore lint/suspicious/noProto: assignment is the behaviour under test
  probe.__proto__ = { probe: true };
  return Object.getPrototypeOf(probe) !== Object.prototype;
})();
...
describe("PackageExtensions — known-defect regression cover", () => {
  it("DEFECT: __proto__ as a dependency name replaces the result object's prototype", () => {
    ...
    // assignment is never rejected. The name is not validated either way, but
    // where it lands depends on the runtime.
    //
    // On Node.js the Object.prototype.__proto__ setter runs and the dependencies
    // object ends up inheriting from an attacker-controlled object.
    // JSON.stringify hides it because it only serialises own enumerable
    // properties, so the pollution is observable only through the prototype
    // chain. Under Deno the assignment defines an ordinary own property instead,
    // so nothing is polluted but the manifest gains a dependency literally named
    // __proto__. Both outcomes are pinned, so an upstream fix that rejects the
    // name surfaces as a deliberate test change on either runtime.
    const pe = new PackageExtensions({
      "pe-target": {
    ...
    if (PROTO_SETTER_MUTATES) {
      ...
      // But not as an own property, which is why it does not appear in the
      // serialised tree — the damage is to the prototype chain.
      assert.deepStrictEqual(Object.keys(result.pkg.dependencies), []);
    } else {
      assert.strictEqual(
        Object.getPrototypeOf(result.pkg.dependencies),
        Object.prototype,
      );
      assert.strictEqual(result.pkg.dependencies.polluted, undefined);
      // Here the damage is visible instead: a dependency named __proto__.
      assert.deepStrictEqual(Object.keys(result.pkg.dependencies), [
        "__proto__",
      ]);
    }
