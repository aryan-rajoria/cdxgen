...
import validateName from "./validate-npm-package-name.js";
...
const parseSelector = (key) => {
  if (typeof key !== "string" || !key) {
    throw err(
      `Invalid packageExtensions selector: ${JSON.stringify(key)}`,
      "EEXTENSIONSELECTOR",
    );
  }
  ...
  if (!validForOldPackages && !validForNewPackages) {
    throw err(
      `Invalid package name in packageExtensions selector: "${key}"`,
      "EEXTENSIONSELECTOR",
      { selector: key },
    );
  }
...
const validateExtensionObject = (key, ext) => {
  if (ext === null || typeof ext !== "object" || Array.isArray(ext)) {
    throw err(
      `packageExtensions["${key}"] must be an object`,
      "EEXTENSIONVALUE",
      { selector: key },
    );
  }
  for (const field of Object.keys(ext)) {
    ...
    if (val === null || typeof val !== "object" || Array.isArray(val)) {
      throw err(
        `packageExtensions["${key}"].${field} must be an object`,
        "EEXTENSIONVALUE",
        { selector: key, field },
      );
    }
  ...
  for (const [name, meta] of Object.entries(ext.peerDependenciesMeta || {})) {
    if (meta === null || typeof meta !== "object" || Array.isArray(meta)) {
      throw err(
        ...
        "EEXTENSIONVALUE",
        { selector: key, field: "peerDependenciesMeta", name },
      );
    }
  }
};
...
const applyExtension = (pkg, { key, ext }) => {
    ...
    if (pkg[field] && typeof pkg[field] === "object") {
      next[field] =
        field === "peerDependenciesMeta"
          ? Object.fromEntries(
              Object.entries(pkg[field]).map(([n, m]) => [n, { ...m }]),
            )
          : { ...pkg[field] };
    }
  }
      ...
      for (const existingField of NORMAL_DEP_FIELDS) {
        if (next[existingField] && name in next[existingField]) {
          throw err(
            ...
            "EEXTENSIONDUPDEP",
            { selector: key, field, name, existingField },
          );
        }
      }
  ...
  if (ext.peerDependenciesMeta) {
    ...
    for (const [name, meta] of Object.entries(ext.peerDependenciesMeta)) {
      next.peerDependenciesMeta[name] = {
        ...next.peerDependenciesMeta[name],
        ...meta,
      };
      (applied.peerDependenciesMeta = applied.peerDependenciesMeta || []).push(
        name,
      );
...
const canonicalHash = (packageExtensions) =>
  ssri
    .fromData(canonicalStringify(packageExtensions), { algorithms: ["sha512"] })
    .toString();
...
class PackageExtensions {
  ...
  wouldMatch(name, version) {
    return this.selectors.some(
      (s) => s.name === name && rangeMatches(s.range, version),
    );
  }
  ...
  match(name, version) {
    const matches = this.selectors.filter(
      (s) => s.name === name && rangeMatches(s.range, version),
    );
    if (matches.length > 1) {
      ...
      throw err(
        ...
        "EEXTENSIONCONFLICT",
        { name, version, selectors: matches.map((s) => s.key) },
      );
    }
