  ...
  class VirtualLoader extends cls {
    ...
    async loadVirtual(options = {}) {
      ...
      if (!s.loadedFromDisk && !options.root) {
        const er = new Error(
          "loadVirtual requires existing package-lock.json file",
        );
      ...
      const pkg = await PackageJson.normalize(this.path)
        .then((p) => p.content)
        .catch(() => s.data.packages[""] || {});
      // TODO clean this up
      const {
...
2. npm install`,
        ...
        if (!link.target.parent) {
          await PackageJson.normalize(link.realpath)
            .then((p) => (link.target.package = p.content))
            .catch(() => null);
        }
      }
    }
