import localeCompareFactory from "@isaacs/string-locale-compare";

const localeCompare = localeCompareFactory("en");
...
class Shrinkwrap {
  ...
  get #filenameSet() {
    ...
    return [`${this.path}/package-lock.json`, `${this.path}/yarn.lock`];
  }
  ...
  get resetFiles() {
    ...
    return Promise.all(
      this.#filenameSet.slice(0, 1).map(
        (file) =>
          file &&
          stat(file).then(
            (st) => st.isFile(),
  ...
  async load() {
    ...
    if (data.lockfileVersion > maxLockfileVersion) {
      throw Object.assign(
        new Error(
          `Unsupported lockfileVersion ${data.lockfileVersion}. This npm only supports up to ${maxLockfileVersion}. Please upgrade npm.`,
        ),
    ...
    this.packageExtensionsHash =
      data.packages?.[""]?.packageExtensionsHash || null;
  ...
  add(node) {
    ...
    if (node.resolved === null || node.integrity === null) {
      const { resolved, integrity, version } = this.get(node.path);
      ...
      if (allOk) {
        ...
        const { resolved, integrity } = Shrinkwrap.metaFromNode(
          node,
          this.path,
          this.resolveOptions,
        );
  ...
  commit() {
    ...
    if (hasPatched && this.lockfileVersion < patchedLockfileVersion) {
      log.warn(
        "shrinkwrap",
        `patchedDependencies requires lockfileVersion ${patchedLockfileVersion}; upgrading the lockfile from version ${this.lockfileVersion}.`,
      );
    ...
    const hasExtensionState =
      !this.hiddenLockfile &&
      (this.packageExtensionsHash ||
        this.npmExtensionHash ||
        Object.values(this.data.packages).some(
          (p) => p.packageExtensionsApplied || p.npmExtensionApplied,
        ));
    if (
      hasExtensionState &&
      ...
      log.warn(
        "shrinkwrap",
        `manifest extensions require lockfileVersion ${packageExtensionsLockfileVersion}; upgrading the lockfile from version ${this.lockfileVersion}.`,
      );
      this.lockfileVersion = packageExtensionsLockfileVersion;
    }
  ...
  save(options = {}) {
    ...
    // toString() sets properties as part of commit(), so it must run before
    // the lockfile conversion check below.  Its serialised output is unused
    // here because save() no longer writes.
