...
export function persistDosaiSemanticsReport(
  ...
  try {
    ...
    // Combined methods + data-flow reports for large solutions can exceed the
    // JS maximum string length. The SBOM evidence is already collected by this
    // point, so skip the durable copy instead of losing the whole analysis.
    recordDegradation("dosai.semantics.persistFailed", {
      ecosystem: "csharp",
      tool: "dosai",
      impact: "minor",
 ...
 * properties carry that distinction to BOM consumers.
 *
...
export function addDosaiReachabilityProperties(component, reachability) {
  ...
  const values = {
    "internal:DosaiReachabilityKind": reachability.ReachabilityKind,
    "internal:DosaiReachabilityConfidence": reachability.Confidence,
    "internal:DosaiReachabilityEvidence": (
      reachability.EvidenceKinds || []
    ).join(", "),
    "internal:DosaiReachabilityReasons": (
 ...
 */
export function applyDosaiReachabilityEvidence(methodsSlice, components = []) {
  ...
  for (const reachability of methodsSlice?.PackageReachability || []) {
    ...
    const component = purl && componentsByPurl.get(purl);
    if (!component) {
      continue;
    }
