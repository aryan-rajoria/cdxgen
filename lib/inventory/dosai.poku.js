...
describe("dosai helpers", () => {
  ...
  it("applyDosaiReachabilityEvidence propagates confidence facts to matching components", () => {
    ...
    assert.strictEqual(
      properties["internal:DosaiReachabilityKind"],
      "ExternalCallGraphNode",
    );
    assert.strictEqual(
      properties["internal:DosaiReachabilityConfidence"],
      "Low",
    );
    assert.strictEqual(
      properties["internal:DosaiReachabilityEvidence"],
      "SourceUnresolved",
    );
    assert.ok(
      properties["internal:DosaiReachabilityReasons"].includes(
        "Package assemblies were not available for semantic binding.",
      ),
    );
