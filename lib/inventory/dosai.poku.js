...
describe("dosai helpers", () => {
  ...
  it("translates cdxgen exclude globs the way the atom evidence filter reads them", async () => {
    ...
    // A literal group stays literal, including shell-style ${...}.
    const shellStyle = ["$", "{x,y}"].join("");
    assert.deepStrictEqual(translate(["lib/{only}/**", shellStyle]).patterns, [
      "**/lib/{only}/**",
      `**/${shellStyle}`,
    ]);
  ...
  it("caps the number of exclude patterns passed to dosai", async () => {
    ...
    assert.strictEqual(
      toDosaiExcludePatterns(["x{1..300}"], resolve(tmpdir())).patterns.length,
      1,
    );
  });
