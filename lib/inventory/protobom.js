...
const normalizeObjectWrappedListsForProto = (bomJson) => {
  ...
  // @cdxgen/cdx-proto <= 2.0.3 does not decode the JSON-native `citations`
  // element: Citation.Pointers/Expressions are wrapper messages around a
  // repeated string, but canonical CycloneDX JSON carries bare string arrays.
  // Drop the field before proto encoding so the JSON document still carries the
  // provenance while the binary export does not fail. Scalar-list-wrapper
  // handling landed in cdx-proto's feat/citations-support branch; once a
  // published release includes it, bump the dependency and remove this guard.
  if (Array.isArray(normalizedBomJson.citations)) {
    delete normalizedBomJson.citations;
  }
