...
// carry `plugins/` (jars + launchers) and require Java 21+. This set must stay
...
export function resolvePhpParseBin() {
  if (process.env.PHP_PARSER_BIN) {
    return process.env.PHP_PARSER_BIN;
  }
  const roots = [dirNameStr];
  if (process.env.GLOBAL_NODE_MODULES_PATH) {
    roots.push(dirname(process.env.GLOBAL_NODE_MODULES_PATH));
  }
...
export function resolveDirectAtomBinaryPath() {
  ...
  if (process.env.GLOBAL_NODE_MODULES_PATH) {
    roots.push(process.env.GLOBAL_NODE_MODULES_PATH);
  }
  const candidates = [];
  for (const root of roots) {
...
export function executeAtom(src, args, extra_env = {}) {
  ...
  // Atom requires Java >= 21 (jar-kind platforms only)
