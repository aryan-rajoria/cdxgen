...
import {
  ...
  primeSvelteParser,
  SVELTE_DEFAULT_LIB_ALIAS,
  SVELTE_KIT_VIRTUAL_MODULE_PREFIXES,
  SVELTE_SCRIPT_EXECUTABLE_PACKAGE_MAP,
  SVELTE_STYLE_LANG_PACKAGES,
} from "./svelteUtils.js";
...
const IGNORE_FILE_PATTERN = new RegExp(
  readEnvironmentVariable("ASTGEN_IGNORE_FILE_PATTERN") ||
    "(test|spec|mock|setup-jest|\\.d)\\.(js|ts|tsx)$|(?<!vite\\.|vue\\.|svelte\\.)(conf|config)\\.(js|ts|tsx)$",
  "i",
);
...
const fileToParseableCode = (file) => {
  ...
  if (file.endsWith(".svelte")) {
    // Svelte components have no <template> element — markup sits at the top
    // level — so the Vue blanking path would leave the raw Svelte markup in
    // the Babel input. Mask everything outside the script bodies instead;
    // the mask preserves offsets and newlines, so import line numbers stay
    // true to the original file.
    return maskSvelteScripts(code);
  }
...
/**
 * Read a file and produce its Babel `File` AST. `.svelte` components prefer
 * the optional atom-parsetools template parser (which re-emits the markup as
 * JSX with absolute offsets) and fall back to a Babel parse of the masked
 * script buffer, both position-preserving. Every other extension goes through
 * `fileToParseableCode` unchanged.
 *
 * @param {string} file Absolute file path.
 * @returns {Object} Babel `File` AST.
 */
const parseSourceFileAST = (file) => {
  if (!file.endsWith(".svelte")) {
    return parse(fileToParseableCode(file), babelParserOptions);
  }
  const code = readFileSync(file, "utf-8");
  const maskedSource = maskSvelteScripts(code);
  const templateParser = getSvelteTemplateParser();
  if (templateParser) {
    try {
      return templateParser.parseSvelteFile(file, code, babelParserOptions);
    } catch (err) {
      // A genuinely broken template must not abort the component: parse the
      // masked script buffer, recording the template failure when the
      // fallback entry point is available. If that also throws, fall through
      // to the built-in mask parse below.
      try {
        if (typeof templateParser.parseSvelteScriptBuffer === "function") {
          return templateParser.parseSvelteScriptBuffer(
            file,
            code,
            maskedSource,
            babelParserOptions,
        ...
        // ignore — the tier 1 parse below is the last resort
      }
    }
  }
  return parse(maskedSource, babelParserOptions);
};
...
const addScriptExecutablePackageRef = (
  allImports,
  allExports,
  src,
  file,
  executable,
  executablePackageMap,
) => {
  const executableName = executable?.executableName;
  if (!executableName) {
    return;
  }
  const packageName = executablePackageMap[executableName.toLowerCase()];
  if (packageName) {
...
const parseFileASTTree = (src, file, allImports, allExports) => {
  const ast = parseSourceFileAST(file);
...
/**
 * Walk an npm script value's shell segments and attribute each leading
 * executable to the npm package that provides it, using the given
 * executable→package map plus the `cdx:npm:bin/` fallback for npx-style
 * package hints. Shared by the Angular and Svelte script-evidence passes.
 */
const addScriptExecutableSegmentRefs = (
  allImports,
  allExports,
  src,
  file,
...
const addAngularScriptPackageRefs = (
  ...
  addScriptExecutableSegmentRefs(
    allImports,
    allExports,
    src,
    file,
...
// ---------------------------------------------------------------------------
// Svelte / SvelteKit evidence
// ---------------------------------------------------------------------------

const SVELTE_CONFIG_FILE_NAMES = new Set([
  "svelte.config.cjs",
  "svelte.config.js",
  "svelte.config.mjs",
  "svelte.config.mts",
...
const SVELTE_RUNE_MODULE_EXTENSIONS = [".svelte.js", ".svelte.ts"];

const getFilesByBaseNames = (
  dir,
  fileNames,
  deep,
  result = [],
...
const getLineColumnFromIndex = (content, index) => {
  const clampedIndex = Math.max(
    0,
    Math.min(Number(index) || 0, String(content).length),
  );
  const prefix = content.slice(0, clampedIndex);
...
const hasSvelteComponentEvidence = (allImports) =>
  Object.keys(allImports).some(
    (importName) =>
      importName === "svelte" ||
      importName.startsWith("svelte/") ||
      importName.startsWith("@sveltejs/"),
  );

/**
 * Attribute framework usage recorded inside a Svelte component: the Svelte
 * compiler itself (component presence plus rune usage), the CSS
 * preprocessor required by `<style lang="…">`, packages referenced from
 * style-block `@use`/`@import`/`url()` rules, and `typescript` for
 * `<script lang="ts">`.
 */
const addSvelteComponentRefs = (allImports, allExports, src, file, content) => {
  setSyntheticImportRef(allImports, allExports, src, file, "svelte", [], {
    column: 0,
    line: 1,
  });
  for (const styleBlock of extractSvelteStyleReferences(content)) {
    const stylePackage = SVELTE_STYLE_LANG_PACKAGES[styleBlock.lang || ""];
    if (stylePackage) {
      setSyntheticImportRef(
        allImports,
        allExports,
        ...
        stylePackage,
        [],
        getLineColumnFromIndex(content, styleBlock.start),
      );
    }
    for (const styleReference of styleBlock.references) {
      setAngularPackageRef(
        allImports,
        allExports,
        src,
        ...
        getLineColumnFromIndex(content, styleReference.index),
      );
    }
  }
  for (const scriptLang of extractSvelteScriptLanguages(content)) {
  ...
  for (const rune of findSvelteRunes(content, true)) {
    setSyntheticImportRef(
      allImports,
      allExports,
      src,
      ...
      "svelte",
      [],
      getLineColumnFromIndex(content, rune.index),
    );
  }
};

const addSvelteConfigRefs = (allImports, allExports, src, file, facts) => {
  // `adapter: "netlify"` names an adapter package without importing it; the
  // imported form (`import adapter from "@sveltejs/adapter-node"`) is already
  // collected by the regular config-file parse.
  const adapterName = facts.adapterName;
  if (adapterName) {
    const packageName = adapterName.startsWith("@")
      ? adapterName
      : `@sveltejs/adapter-${adapterName}`;
    setSyntheticImportRef(allImports, allExports, src, file, packageName, [], {
      column: 0,
      line: 1,
    });
  }
...
const normalizeSvelteAliasTarget = (aliasTarget) =>
  String(aliasTarget || "")
    .replaceAll("\\", "/")
    .replace(/^\.\//, "")
    .replace(/\/+$/, "");
...
/**
 * Re-key SvelteKit virtual modules and path aliases so they never surface as
 * junk package names: `$app/*`, `$env/*`, and `$service-worker` become
 * `@sveltejs/kit` evidence (the package that provides those modules), and
 * `$lib/*` plus any configured `$alias/*` become relative file references.
 * Original occurrences are kept, so file and line evidence survives.
 */
const reattributeSvelteKitModules = (allImports, _allExports, kitAliases) => {
  const aliasEntries = Object.entries(kitAliases || {}).filter(([aliasKey]) =>
    aliasKey.startsWith("$"),
  );
  for (const importKey of Object.keys(allImports)) {
    const isVirtualModule = SVELTE_KIT_VIRTUAL_MODULE_PREFIXES.some(
      (prefix) => importKey === prefix || importKey.startsWith(`${prefix}/`),
    );
    const aliasEntry = aliasEntries.find(
      ([aliasKey]) =>
        importKey === aliasKey || importKey.startsWith(`${aliasKey}/`),
    );
    if (!isVirtualModule && !aliasEntry) {
      continue;
    }
    const occurrences = allImports[importKey] || [];
    const targetKey = isVirtualModule
      ? "@sveltejs/kit"
      : aliasEntry
        ? importKey === aliasEntry[0]
          ? normalizeSvelteAliasTarget(aliasEntry[1])
          : `${normalizeSvelteAliasTarget(aliasEntry[1])}/${importKey.slice(aliasEntry[0].length + 1)}`
        : undefined;
    if (!targetKey) {
      delete allImports[importKey];
      continue;
    }
    allImports[targetKey] = allImports[targetKey] || new Set();
    for (const occurrence of occurrences) {
      if (isVirtualModule) {
        allImports[targetKey].add(occurrence);
      } else {
        // A path alias resolves inside the project, never to a package.
        allImports[targetKey].add({
          ...occurrence,
          importedAs: importKey,
          isExternal: false,
        });
      }
    }
...
/**
 * Collect Svelte framework evidence after the main parse fan-out: component
 * facts (runes, style/script languages, style package references),
 * `svelte.config.*` facts (string-form adapter, `kit.alias`), npm script
 * executables (`svelte-kit sync`, `svelte-check`, ...), and re-attribution
 * of SvelteKit virtual modules. Returns the resolved alias map (or undefined
 * when the project shows no Svelte evidence at all).
 *
 * `srcFiles` is the file list the parse fan-out already walked, so the Svelte
 * pass costs no extra directory traversal on projects that turn out not to be
 * Svelte projects at all.
 */
const parseSvelteEvidenceFiles = (
  src,
  allImports,
  allExports,
  searchOptions,
  ...
  const runeModuleFiles = [];
  for (const srcFile of srcFiles || []) {
    if (srcFile.endsWith(".svelte")) {
      componentFiles.push(srcFile);
    } else if (
      SVELTE_RUNE_MODULE_EXTENSIONS.some((extension) =>
        srcFile.endsWith(extension),
      )
    ) {
      runeModuleFiles.push(srcFile);
    }
  }
  if (
    !componentFiles.length &&
    !runeModuleFiles.length &&
    !hasSvelteComponentEvidence(allImports)
  ) {
    return undefined;
  }
  for (const componentFile of componentFiles) {
    let content;
    try {
      content = readFileSync(componentFile, "utf-8");
    } catch {
      continue;
    }
    addSvelteComponentRefs(allImports, allExports, src, componentFile, content);
  }
  for (const runeModuleFile of runeModuleFiles) {
    let content;
    try {
      content = readFileSync(runeModuleFile, "utf-8");
    } catch {
      continue;
    }
    for (const rune of findSvelteRunes(content, false)) {
      setSyntheticImportRef(
        allImports,
        allExports,
        src,
  ...
  const configAndManifestFiles = getFilesByBaseNames(
    src,
    new Set([...SVELTE_CONFIG_FILE_NAMES, "package.json"]),
    searchOptions.deep,
    [],
    src,
    searchOptions.exclude,
  ...
  const kitAliases = {};
  for (const evidenceFile of configAndManifestFiles) {
    const evidenceFileName = basename(evidenceFile);
    let content;
    try {
      content = readFileSync(evidenceFile, "utf-8");
    ...
    if (SVELTE_CONFIG_FILE_NAMES.has(evidenceFileName)) {
      const facts = parseSvelteConfigSource(content);
      addSvelteConfigRefs(allImports, allExports, src, evidenceFile, facts);
      Object.assign(kitAliases, facts.aliases);
    } else {
      let packageJson;
      try {
  ...
  if (!kitAliases.$lib) {
    kitAliases.$lib = SVELTE_DEFAULT_LIB_ALIAS;
  }
  reattributeSvelteKitModules(allImports, allExports, kitAliases);
  return kitAliases;
};
...
export const findJSImportsExports = async (src, deep) => {
  ...
  try {
    const searchOptions = normalizeAnalyzerSearchOptions(deep);
    // Resolve the optional Svelte template parser before the fan-out so the
    // inline path can use it; worker threads resolve it lazily at module
    // scope of astWorker.js. Absence is silent — tier 1 covers all scope
    // evidence.
    await primeSvelteParser();
