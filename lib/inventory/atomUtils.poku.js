...
it("passes the heap ceiling to atom ahead of its own arguments", () => {
  ...
  const argvPath = join(tmpdir(), `atom-stub-argv-${process.pid}.json`);
  writeFileSync(
    stubPath,
  ...
  try {
    ...
    const argv = JSON.parse(readFileSync(argvPath, "utf-8"));
...
it("warns once when the heap is below what slicing is comfortable with", () => {
  ...
  // Once for the process, not once per spawn: atom runs once per language and
  // the heap is identical every time.
  assert.strictEqual(tight.length, 1);
  assert.ok(tight[0].includes("4 GiB"));
  assert.ok(tight[0].includes("7 GiB"));
  assert.ok(tight[0].includes("ATOM_MAX_HEAP"));
});
