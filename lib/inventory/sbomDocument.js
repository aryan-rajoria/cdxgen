...
export function parseSbomDocument(
  ...
  for (const component of parsed.components) {
    repairDownloadUrlQualifier(component);
  }
  return parsed;
}

/**
 * Repair the `download_url` qualifier of a component's purl.
 *
 * Producers write this qualifier as a bare URL, but a purl qualifier value is
 * percent-encoded, so an unescaped `://` makes the whole purl unparseable. A
 * `file:` URL is dropped outright: it names a path on the machine that built
 * the document, which identifies nothing for a reader of the BOM.
 *
 * @param {object} component Component whose purl is rewritten in place
 */
function repairDownloadUrlQualifier(component) {
  const purl = component?.purl;
  if (typeof purl !== "string" || !purl.includes("download_url=")) {
    return;
  }
