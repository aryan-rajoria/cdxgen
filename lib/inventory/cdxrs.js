...
/**
 * Spec versions `cdxrs validate` accepts, as reported by `cdxrs schema-version`.
 * The JS validator additionally accepts the experimental 2.0-dev output.
 */
const CDXRS_VALIDATE_SPEC_VERSIONS = new Set(["1.6", "1.7"]);
 ...
 */
export function cdxrsCanValidate(bomJson) {
  if (!cdxrsAvailable("validate").available) {
    return false;
  }
  return CDXRS_VALIDATE_SPEC_VERSIONS.has(String(bomJson?.specVersion ?? ""));
}
