...
export async function prefetchJson(requests, options = {}) {
  ...
  // The cdxrs envelope types a result body as JSON, so a batch that wants text
  // or bytes goes to the JS pool, which passes `responseType` straight to the
  // agent. The pool applies the same host concurrency and rate policy, so the
  // batching win is unaffected; only the on-disk cache is skipped.
  if (batchFetchAvailable() && unique.every(isJsonRequest)) {
...
/**
 * Whether a request expects a JSON body, and so can be served by cdxrs.
 *
 * @param {{responseType?: string}} request
 * @returns {boolean}
 */
