...
export async function prefetchJson(requests, options = {}) {
  ...
  if (batchFetchAvailable() && unique.every(cdxrsCanServe)) {
 ...
 * - anything addressed to crates.io, whose crawler policy allows one request
 *   per second with no parallelism. Honouring that needs a single gate over
 *   the whole process, and the two transports keep separate ones: a run that
 *   split a crates batch across both would issue two requests per second. The
 *   JS pool owns crates.io so there is one gate to reason about.
 *
...
function cdxrsCanServe(request) {
  ...
  if (isCratesHost(extractHost(request.url))) {
    return false;
  }
 ...
 */
export async function withHostRateLimit(url, issue) {
