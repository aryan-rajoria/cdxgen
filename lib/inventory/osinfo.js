...
/**
 * Vendor prefixes that `distro` qualifier values are canonicalised for.
 *
 * This is deliberately Azure-only for now: other distros keep their
 * historical qualifier spellings (e.g. trivy's `oracle-9.8` next to
 * os-release-derived `ol-9.8`) until the vulnerability-data side is aligned,
 * so canonicalising them here would silently change matching for distros
 * outside this fix. Keys are matched longest-first so `cbl-mariner-2.0`
 * resolves against the full vendor segment.
 */
