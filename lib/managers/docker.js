...
import { getDirs, getOnlyDirs, getOnlyDirsMulti } from "./containerutils.js";
import { createDaemonConnection } from "./dockerConnection.js";
...
const resolveRequestedRegistryRef = (forRegistry, requestedRegistryRef) => {
  const fallbackRegistry =
    forRegistry ||
    readEnvironmentVariable("DOCKER_SERVER_ADDRESS") ||
    DOCKER_HUB_REGISTRY;
  if (
...
let isContainerd = !!readEnvironmentVariable("CONTAINERD_ADDRESS");
...
if (
  !readEnvironmentVariable("DOCKER_HOST") &&
  (readEnvironmentVariable("CONTAINERD_ADDRESS") ||
    (readEnvironmentVariable("XDG_RUNTIME_DIR") &&
      safeExistsSync(
        join(
          readEnvironmentVariable("XDG_RUNTIME_DIR"),
          "containerd-rootless",
 ...
 */
export const getConnection = async (_options, forRegistry) => {
  ...
  if (!dockerConn) {
    ...
    // Probe a candidate daemon endpoint. Returns a live connection on success
    // and closes/discards the connection (throwing) on failure so callers can
    // try the next candidate.
    const tryConnect = async (prefixUrl, pingPath) => {
      const conn = createDaemonConnection(
        prefixUrl,
        defaultOptions.headers,
        defaultOptions.https,
      );
      try {
        await conn.request(pingPath, { method: "GET" });
        return conn;
      } catch (err) {
        await conn.close();
        throw err;
    ...
    try {
      dockerConn = await tryConnect(defaultOptions.prefixUrl, "_ping");
      if (DEBUG_MODE) {
        if (isDockerRootless) {
      ...
      try {
        if (isWin) {
          dockerConn = await tryConnect(WIN_LOCAL_TLS, "_ping");
          isWinLocalTLS = true;
          if (DEBUG_MODE) {
            console.log("Docker desktop on Windows detected.");
          }
        } else {
          dockerConn = await tryConnect(podmanRootlessUrl, "libpod/_ping");
          isPodman = true;
        ...
        try {
          dockerConn = await tryConnect(podmanRootUrl, "libpod/_ping");
          isPodman = true;
...
export const makeRequest = async (path, method, forRegistry) => {
  ...
  // The connection is already bound to the correct daemon base URL. Only pass
  // per-request auth headers and the method.
...
const IMAGE_DIGEST_ALGO_RE = /^[a-z0-9]+(?:[.+_-][a-z0-9]+)*$/;
const IMAGE_DIGEST_HEX_RE = /^[a-f0-9]{32,}$/;
const IMAGE_COMPONENT_RE = /^[a-z0-9]+(?:[._-]+[a-z0-9]+)*$/;
const IMAGE_HOST_PORT_RE = /^\d{1,5}$/;
// A single label with a bounded quantifier and no alternation: checking hosts
// label by label keeps the cost linear on hostile inputs, which the equivalent
// whole-host pattern cannot promise (its nested quantifiers backtrack
// exponentially on strings such as "0.0" followed by many repetitions of "0").
const IMAGE_HOST_LABEL_RE = /^[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?$/;

/**
 * Validate the registry host of an image reference: a dotted name, `localhost`,
 * or a bare name with a numeric port, each label 1-63 characters of letters,
 * digits and inner hyphens.
 *
 * @param {string} host Host component without the trailing slash.
 * @returns {boolean} true when the host is well-formed.
 */
const isValidImageHost = (host) => {
  let name = host;
  let hasPort = false;
  const colonIndex = name.lastIndexOf(":");
  if (colonIndex !== -1) {
    const port = name.slice(colonIndex + 1);
    if (!IMAGE_HOST_PORT_RE.test(port)) {
      return false;
    }
  ...
  if (!name.length || name.length > 253) {
    return false;
  }
  const labels = name.split(".");
  if (labels.length < 2 && !hasPort) {
    return false;
  }
  return labels.every((label) => IMAGE_HOST_LABEL_RE.test(label));
};
 ...
 */
export const isImageReference = (fullImageName) => {
  if (typeof fullImageName !== "string") {
    return false;
  }
  const ref = fullImageName.trim().toLowerCase();
  if (!ref.length || ref.length > 1024) {
    return false;
  }
  if (
    ref.includes("\\") ||
  ...
  if (atIndex !== -1) {
    const digest = remainder.slice(atIndex + 1);
    remainder = remainder.slice(0, atIndex);
    if (remainder.includes("@") || !remainder.length) {
      return false;
    }
    const digestColonIndex = digest.indexOf(":");
    if (
      digestColonIndex <= 0 ||
      digestColonIndex === digest.length - 1 ||
      !IMAGE_DIGEST_ALGO_RE.test(digest.slice(0, digestColonIndex)) ||
      !IMAGE_DIGEST_HEX_RE.test(digest.slice(digestColonIndex + 1))
    ) {
      return false;
    }
  }
  ...
  if (colonIndex > slashIndex) {
    const tag = remainder.slice(colonIndex + 1);
  ...
  return components.every(
    (component) =>
      ...
      IMAGE_COMPONENT_RE.test(component),
  );
};
...
const getContainerCliCmd = () => {
  if (readEnvironmentVariable("DOCKER_CMD")?.trim()) {
    return readEnvironmentVariable("DOCKER_CMD").trim();
  }
...
const needsCliFallback = () => {
  if (
    ["true", "1"].includes(readEnvironmentVariable("DOCKER_USE_CLI")) ||
    readEnvironmentVariable("DOCKER_CMD")?.trim() ||
  ...
  return (
    isWin ||
    (readEnvironmentVariable("DOCKER_HOST") &&
      (readEnvironmentVariable("DOCKER_HOST").startsWith("tcp://") ||
        readEnvironmentVariable("DOCKER_HOST").startsWith("ssh://")))
  );
};
...
export const extractTar = async (fullImageName, dir, options) => {
  try {
    ...
    deferFailOnError(options, {
      ecosystem: "container",
      tool: "docker",
      detail: "extracting the image archive failed",
    });
    return false;
  }
};
...
export const exportArchive = async (fullImageName, options = {}) => {
  ...
  try {
    ...
    deferFailOnError(options, {
      ecosystem: "container",
      tool: "docker",
      detail: "the image archive could not be extracted",
    });
  } catch (_err) {
    // ignore
    deferFailOnError(options, {
...
export const extractFromManifest = async (
  ...
  if (Array.isArray(manifest)) {
    ...
    const layerPhase = ui.phase("Extracting image layers");
    let extractedLayers = 0;
    for (const layer of layers) {
      layerPhase.progress(extractedLayers, layers.length);
      layerPhase.detail(basename(layer));
      extractedLayers += 1;
      try {
      ...
      try {
        ...
        if (err.code === "TAR_BAD_ARCHIVE") {
          ...
          deferFailOnError(options, {
            ecosystem: "container",
            tool: "docker",
            detail: "reading the image manifest failed",
          });
        }
      }
    }
...
export const getPkgPathList = (exportData, lastWorkingDir) => {
  ...
  */
  ...
  for (const wpath of knownSysPaths) {
    ...
    const ecoDirs = getOnlyDirsMulti(wpath, ecoDirNames);
    if (ecoDirs.node_modules.length) {
      pathList.push(ecoDirs.node_modules[0]);
    }
    if (ecoDirs["site-packages"].length) {
      pathList = pathList.concat(ecoDirs["site-packages"]);
    }
    if (ecoDirs.gems.length) {
      pathList.push(ecoDirs.gems[0]);
    }
    if (ecoDirs[".cargo"].length) {
      pathList = pathList.concat(ecoDirs[".cargo"]);
    }
    if (ecoDirs[".composer"].length) {
      pathList = pathList.concat(ecoDirs[".composer"]);
    }
  }
