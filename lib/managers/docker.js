...
const IMAGE_HOST_RE =
  /^(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?(?:\.[a-z0-9](?:[a-z0-9-]*[a-z0-9])*)+|localhost)(?::\d{1,5})?$/;
const IMAGE_HOST_PORT_ONLY_RE = /^[a-z0-9][a-z0-9-]*:\d{1,5}$/;
 ...
 * numeric port, or is exactly localhost. The per-piece regexes are linear and
 * the input is length-capped, so hostile inputs cannot backtrack expensively.
 *
...
export const isImageReference = (fullImageName) => {
  ...
  if (components.length > 1) {
    ...
    if (looksLikeHost) {
      if (!IMAGE_HOST_RE.test(host) && !IMAGE_HOST_PORT_ONLY_RE.test(host)) {
        return false;
      }
      components.shift();
    }
