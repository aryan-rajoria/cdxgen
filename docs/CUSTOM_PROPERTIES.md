...
| `cdx:cargo:*`                                                                              | Rust crates.io                                                                      | Crate metadata linkage (id/latest/rust version/features), and `metadataSource` naming where the metadata came from                                                                                           | Validate Rust toolchain compatibility, flag feature-driven attack surface changes, monitor lag from newest upstream version; `metadataSource=local-registry` means the local Cargo registry answered and publisher provenance was not collected | [Package manager and language ecosystems](#inventory-packages)                                            | [4](#example-4)                                                |
...
| `cdx:julia:*`                                                                              | Julia (`Project.toml`, `Manifest.toml`)                                             | Package UUID, direct vs transitive origin, standard-library marking                                                                                                                                          | Separate Julia's bundled libraries from registry packages, match packages by UUID across registries                                                                                                                                             | [Package manager and language ecosystems](#inventory-packages)                                            | -                                                              |
| `cdx:renv:*`                                                                               | R (`renv.lock`)                                                                     | Source repository category, repository name, renv DESCRIPTION hash, and the R version on the parent                                                                                                          | Distinguish repo-sourced from GitHub-sourced packages; the hash traces the DESCRIPTION, not the artifact bytes                                                                                                                                  | [Package manager and language ecosystems](#inventory-packages)                                            | -                                                              |
| `cdx:rebar:*`                                                                              | Erlang rebar3 (`rebar.lock`)                                                        | Direct vs transitive origin from the lock's dependency level                                                                                                                                                 | Scope review to the project's own hex dependencies; content digests live in `hashes[]`                                                                                                                                                          | [Package manager and language ecosystems](#inventory-packages)                                            | -                                                              |
| `cdx:opam:*`                                                                               | OCaml opam (`.opam.locked`)                                                         | Whether a package is pinned and its sanitised pin source (origin and path only)                                                                                                                              | Spot registry divergence: a pin overrides where a package is fetched from                                                                                                                                                                       | [Package manager and language ecosystems](#inventory-packages)                                            | -                                                              |
| `cdx:tf:*`                                                                                 | Terraform and OpenTofu (`.terraform.lock.hcl`)                                      | Provider registry address, the version constraints that produced the pin, and the `h1:` content digest                                                                                                       | Correlate providers with registries and audit constraint drift; `zh:` package digests are the artifact checksums and live in `hashes[]`                                                                                                         | [Package manager and language ecosystems](#inventory-packages)                                            | -                                                              |
| `cdx:stack:*`                                                                              | Haskell Stack (`stack.yaml`, `stack.yaml.lock`)                                     | Snapshot the project resolves against, and for each extra-dep its repository origin and pinned commit                                                                                                        | Review the extra-deps that step outside the snapshot, and trace repository-sourced packages to an exact commit                                                                                                                                  | [Package manager and language ecosystems](#inventory-packages)                                            | -                                                              |
| `cdx:cpan:*`                                                                               | Perl Carton (`cpanfile`, `cpanfile.snapshot`)                                       | Direct vs transitive origin across distributions                                                                                                                                                             | Scope review to requirements declared in `cpanfile`                                                                                                                                                                                             | [Package manager and language ecosystems](#inventory-packages)                                            | -                                                              |
| `cdx:luarocks:*`                                                                           | Lua (`*.rockspec`, `luarocks.lock`)                                                 | Direct vs transitive origin of resolved rocks                                                                                                                                                                | Scope review to rocks declared in the rockspec                                                                                                                                                                                                  | [Package manager and language ecosystems](#inventory-packages)                                            | -                                                              |
| `cdx:crystal:*`                                                                            | Crystal (`shard.yml`, `shard.lock`)                                                 | Language version constraint, direct vs transitive or dev origin, development scope                                                                                                                           | Separate runtime from test-only shards; review the required Crystal version                                                                                                                                                                     | [Package manager and language ecosystems](#inventory-packages)                                            | -                                                              |
| `cdx:elm:*`                                                                                | Elm (`elm.json`, `elm-package.json`, `elm-stuff/exact-dependencies.json`)           | Language version on the project, direct vs indirect or test origin per package, and the declared version range when no resolved version is known                                                             | Separate runtime from test-only packages; review the required Elm version and which constraints remain unresolved                                                                                                                               | [Package manager and language ecosystems](#inventory-packages)                                            | -                                                              |
| `cdx:nim:*`                                                                                | Nim (`*.nimble`, `nimble.lock`)                                                     | Direct vs transitive origin, pinned git revision                                                                                                                                                             | Trace URL-sourced packages to the exact commit                                                                                                                                                                                                  | [Package manager and language ecosystems](#inventory-packages)                                            | -                                                              |
| `cdx:spack:*`                                                                              | Spack (`spack.lock`)                                                                | Spack DAG hash, package namespace, target architecture, and compiler                                                                                                                                         | Identify the concrete build (variants, compiler and architecture included); the DAG hash is an identity, not an artifact digest                                                                                                                 | [Package manager and language ecosystems](#inventory-packages)                                            | -                                                              |
| `cdx:gradle:catalog`                                                                       | Gradle version catalog (`gradle/libs.versions.toml`)                                | Marks components emitted from the catalog as declared, not resolved                                                                                                                                          | Keep declared catalog entries out of resolved-graph conclusions; emitted only when Gradle produced no dependency information                                                                                                                    | [Package manager and language ecosystems](#inventory-packages)                                            | -                                                              |
| `cdx:vcpkg:*`                                                                              | C/C++ vcpkg (`vcpkg.json`)                                                          | Builtin baseline commit on the parent, declared marking and requested features per port                                                                                                                      | Tie the manifest to the port version set; feature names expand what a port builds                                                                                                                                                               | [Package manager and language ecosystems](#inventory-packages)                                            | -                                                              |
| `cdx:xmake:*`                                                                              | C/C++ xmake (`xmake-requires.lock`)                                                 | Build configurations (`platform                                                                                                                                                                              | arch`) a package was resolved for                                                                                                                                                                                                               | See which platforms actually pull a package in; a package resolved for one platform is not in every build | [Package manager and language ecosystems](#inventory-packages) | -   |
...
- **Rust:** `cdx:cargo:artifactDigestSha256`, `cdx:cargo:binNames`, `cdx:cargo:buildDependencyNames`, `cdx:cargo:buildScript`, `cdx:cargo:buildScriptCapabilities`, `cdx:cargo:buildScriptIndicators`, `cdx:cargo:compressedCadence`, `cdx:cargo:crate_id`, `cdx:cargo:crateSize`, `cdx:cargo:defaultFeatures`, `cdx:cargo:dependencyFeatures`, `cdx:cargo:activated`, `cdx:cargo:alternateRegistry`, `cdx:cargo:dependencyKind`, `cdx:cargo:dependencyKinds`, `cdx:cargo:edition`, `cdx:cargo:features`, `cdx:cargo:git`, `cdx:cargo:gitBranch`, `cdx:cargo:gitRev`, `cdx:cargo:gitTag`, `cdx:cargo:hasBuildDependencies`, `cdx:cargo:hasBuildScript`, `cdx:cargo:hasDevDependencies`, `cdx:cargo:hasLib`, `cdx:cargo:hasNativeBuild`, `cdx:cargo:hasTargetDependencies`, `cdx:cargo:hostOnly`, `cdx:cargo:latest_version`, `cdx:cargo:linkage`, `cdx:cargo:linkageEvidence`, `cdx:cargo:links`, `cdx:cargo:manifestMode`, `cdx:cargo:metadataSource`, `cdx:cargo:nativeLibrary`, `cdx:cargo:nativeBuildIndicators`, `cdx:cargo:optional`, `cdx:cargo:ownerSet`, `cdx:cargo:ownerSetCount`, `cdx:cargo:package`, `cdx:cargo:packageCreatedTime`, `cdx:cargo:path`, `cdx:cargo:priorPublishTime`, `cdx:cargo:priorPublisher`, `cdx:cargo:priorVersion`, `cdx:cargo:procMacro`, `cdx:cargo:provenanceDigest`, `cdx:cargo:provenanceKeyId`, `cdx:cargo:provenancePredicateType`, `cdx:cargo:provenanceSignature`, `cdx:cargo:providedBy`, `cdx:cargo:provenanceUrl`, `cdx:cargo:publishTime`, `cdx:cargo:publisher`, `cdx:cargo:publisherDrift`, `cdx:cargo:publisherOverlapCount`, `cdx:cargo:publisherOverlapRatio`, `cdx:cargo:publisherSet`, `cdx:cargo:publisherSetCount`, `cdx:cargo:registry`, `cdx:cargo:releaseCadenceCompressionRatio`, `cdx:cargo:releaseGapBaselineDays`, `cdx:cargo:releaseGapDays`, `cdx:cargo:releaseGapSampleSize`, `cdx:cargo:releaseProfiles`, `cdx:cargo:resolvedFeatures`, `cdx:cargo:sourceKind`, `cdx:cargo:sourceUrl`, `cdx:cargo:rust_version`, `cdx:cargo:rustVersion`, `cdx:cargo:scope`, `cdx:cargo:filterPlatform`, `cdx:cargo:target`, `cdx:cargo:targetExcluded`, `cdx:cargo:targetGated`, `cdx:cargo:trustedPublishing`, `cdx:cargo:upstreamVersionCandidates`, `cdx:cargo:upstreamVersionSource`, `cdx:cargo:usesMaturin`, `cdx:cargo:versionCount`, `cdx:cargo:workspaceDependency`, `cdx:cargo:workspaceMembers`, `cdx:cargo:workspaceRoot`, `cdx:cargo:yanked`, `cdx:maturin:*`, `cdx:rust:buildTool`
- **Evidence based filtering:** `cdx:evidence:usage`
- **Rusi Rust Evinse:** `cdx:rusi:schemaVersion`, `cdx:rusi:toolVersion`, `cdx:rusi:rustcVersion`, `cdx:rusi:cargoVersion`, `cdx:rusi:host`, `cdx:rusi:backend`, `cdx:rusi:requestedBackend`, `cdx:rusi:analysisScope`, `cdx:rusi:callGraphMode`, `cdx:rusi:dataFlowMode`, `cdx:rusi:packageCount`, `cdx:rusi:fileCount`, `cdx:rusi:importCount`, `cdx:rusi:declarationCount`, `cdx:rusi:usageCount`, `cdx:rusi:securitySignalCount`, `cdx:rusi:cryptoLibraryCount`, `cdx:rusi:cryptoComponentCount`, `cdx:rusi:cryptoFindingCount`, `cdx:rusi:callGraphNodeCount`, `cdx:rusi:callGraphEdgeCount`, `cdx:rusi:dataFlowNodeCount`, `cdx:rusi:dataFlowEdgeCount`, `cdx:rusi:dataFlowSliceCount`, `cdx:rusi:importPath`, `cdx:rusi:importAlias`, `cdx:rusi:importedBy`, `cdx:rusi:usageKind`, `cdx:rusi:dataFlowCategories`, `cdx:rusi:dataFlowRuleName`, `cdx:rusi:securitySignalCategory`, `cdx:rusi:securitySignalSeverity`, `cdx:rusi:cryptoFindingCategory`, `cdx:rusi:cryptoFindingSeverity`
...
- **Julia:** `cdx:julia:uuid`, `cdx:julia:dependency`, `cdx:julia:stdlib`
- **R (renv):** `cdx:renv:source`, `cdx:renv:repository`, `cdx:renv:hash`, `cdx:renv:rVersion`
- **Erlang (rebar3):** `cdx:rebar:dependency`
- **OCaml (opam):** `cdx:opam:pinned`, `cdx:opam:pinSource`
- **Terraform / OpenTofu:** `cdx:tf:address`, `cdx:tf:constraints`, `cdx:tf:h1`
- **Haskell (Stack):** `cdx:stack:snapshot`, `cdx:stack:dependency`, `cdx:stack:repository`, `cdx:stack:commit`
- **Perl (Carton):** `cdx:cpan:dependency`
- **Lua (LuaRocks):** `cdx:luarocks:dependency`
- **Crystal (shards):** `cdx:crystal:languageVersion`, `cdx:crystal:dependency`, `cdx:crystal:scope`
- **Elm:** `cdx:elm:elmVersion`, `cdx:elm:dependency`, `cdx:elm:versionRange`
- **Nim (nimble):** `cdx:nim:dependency`, `cdx:nim:vcsRevision`
- **Spack:** `cdx:spack:namespace`, `cdx:spack:hash`, `cdx:spack:arch`, `cdx:spack:compiler`
- **Gradle version catalog:** `cdx:gradle:catalog`
- **vcpkg:** `cdx:vcpkg:baseline`, `cdx:vcpkg:declared`, `cdx:vcpkg:features`
...
- **xmake:** `cdx:xmake:configurations`
...
| `cdx:cargo:dependencyKinds`               | component             | list string                     | `dev, runtime`                                                             | When a crate is reached by edges of more than one kind                                                                                            | Shows that a crate serves both a shipped and a test-only purpose                                                                                                                                                                                                                                      | Warning / triage |
| `cdx:cargo:targetGated`                   | component             | boolean string                  | `"true"`                                                                   | When every path from a workspace member to the crate crosses a `cfg()` gate                                                                       | Identifies crates that no build for an unmatched target triple pulls in                                                                                                                                                                                                                               | Warning / triage |
| `cdx:cargo:targetExcluded`                | component             | boolean string                  | `"true"`                                                                   | When `CDXGEN_CARGO_TARGET` is set and the crate is absent from the resolve graph for that triple                                                  | Identifies crates that no build for the requested target triple links, so a cross-compiled product can be described without them                                                                                                                                                                      | Warning / triage |
| `cdx:cargo:filterPlatform`                | component             | target triple string            | `armv5te-unknown-linux-gnueabi`                                            | Alongside `cdx:cargo:targetExcluded`, naming the triple the crate was excluded for                                                                | Records which target the exclusion was decided against                                                                                                                                                                                                                                                | Informational    |
| `cdx:cargo:hostOnly`                      | formulation component | boolean string                  | `"true"`                                                                   | When the crate is a build dependency or a proc-macro                                                                                              | Separates crates compiled for the build host from those linked into the shipped artifact. These components are moved from `components[]` to the `formulation` section                                                                                                                                 | Warning / triage |
| `cdx:cargo:procMacro`                     | component             | boolean string                  | `"true"`                                                                   | When `cargo metadata` reports a `proc-macro` target for the crate                                                                                 | Flags compile-time code execution                                                                                                                                                                                                                                                                     | Warning / triage |
| `cdx:cargo:activated`                     | component             | boolean string                  | `"false"`                                                                  | When the crate is locked but absent from the resolve graph, so no enabled feature pulls it in                                                     | Distinguishes a locked optional dependency from one that is actually built                                                                                                                                                                                                                            | Warning / triage |
| `cdx:cargo:resolvedFeatures`              | component             | list string                     | `default, derive, std`                                                     | With `--deep`, the features `cargo metadata`'s resolve graph reports as enabled                                                                   | Says which code is actually compiled, which optional dependencies activate, and whether a vulnerable feature path exists in this build at all                                                                                                                                                         | Warning / triage |
| `cdx:cargo:links`                         | component             | library name string             | `openssl`                                                                  | When the crate declares `links` in its manifest, which a `-sys` crate does                                                                        | Marks the crates that put a native C library into the artifact                                                                                                                                                                                                                                        | Warning / triage |
| `cdx:cargo:nativeLibrary`                 | component             | library name string             | `z`                                                                        | On a native library component, naming the crate's `links` value it was derived from                                                               | Ties the emitted library back to the linker-level name the crate declared                                                                                                                                                                                                                             | Informational    |
| `cdx:cargo:providedBy`                    | component             | purl                            | `pkg:cargo/openssl-sys@0.9.117`                                            | On a native library component, naming the `-sys` crate that ships it                                                                              | Identifies which crate to update to pick up a fix in the native library                                                                                                                                                                                                                               | Warning / triage |
| `cdx:cargo:linkage`                       | component             | enum string                     | `static`, `dynamic`                                                        | On a native library component, from the vendoring features the resolver enabled                                                                   | `static` means the library ships inside the artifact and its advisories and license obligations travel with it; `dynamic` means the deployment environment must provide and patch it, and the component is also marked `isExternal` on CycloneDX 1.7+                                                 | Hard deny        |
| `cdx:cargo:linkageEvidence`               | component             | list string, repeatable         | `vendored`, `openssl-src@300.6.1+3.6.3`                                    | Alongside `cdx:cargo:linkage`, naming the feature or source crate that decided it                                                                 | Lets a reviewer confirm the linkage conclusion instead of taking it on trust                                                                                                                                                                                                                          | Informational    |
| `cdx:cargo:upstreamVersionSource`         | component             | crate id or file path string    | `libz-sys-1.1.29/src/zlib/zlib.h`                                          | On a native library component whose version was recovered, naming where from                                                                      | States whether the version came from a crate version's build metadata or from a define in the vendored C source                                                                                                                                                                                       | Informational    |
| `cdx:cargo:upstreamVersionCandidates`     | component             | list string                     | `1.3.1, 2.2.2.zlib-ng`                                                     | When the crate ships several variants of the library and the enabled features do not say which is compiled                                        | The component is deliberately left versionless; the variants can carry different licenses, so a license review cannot finish from the BOM alone                                                                                                                                                       | Warning / triage |
| `cdx:cargo:sourceKind`                    | component             | enum string                     | `git`, `local`, `registry`                                                 | When the crate's source is not plain crates.io                                                                                                    | Flags non-default provenance: a git source, a local path or `[patch]`/`[replace]` override, or an alternative registry                                                                                                                                                                                | Warning / triage |
| `cdx:cargo:sourceUrl`                     | component             | URL string                      | `https://cargo.internal.example.com/index`                                 | Alongside `cdx:cargo:sourceKind`                                                                                                                  | Names the registry or repository the crate was resolved from                                                                                                                                                                                                                                          | Warning / triage |
| `cdx:cargo:alternateRegistry`             | component             | boolean string                  | `"true"`                                                                   | When the crate came from a registry other than crates.io                                                                                          | A private registry can publish any name and version, including ones that also exist on crates.io; the purl carries the registry as a `repository_url` qualifier so the identities do not collide                                                                                                      | Hard deny        |
| `cdx:evidence:usage`                      | component             | enum string                     | `occurrence`, `callstack`                                                  | When `--required-only` runs after the evidence stage and the component was observed in the analyzed sources                                       | Records that the component survived the required-only filter on observed usage rather than on its declared scope                                                                                                                                                                                      | Warning / triage |
...
| `cdx:nuget:declared_version_range`        | component             | string                          | `[3.13.3,4.0)`, `2.0.*`, `$(JsonVersion)`                                  | When a .NET manifest states a version constraint rather than an installed version - a NuGet range, a wildcard, or an unresolved MSBuild property  | Keeps the declaration visible while the component stays versionless, so a scanner never sees a `version` field its purl cannot back up                                                                                                                                                                | Context only     |
| `cdx:nuget:metadata_version`              | component             | version string                  | `13.0.4`                                                                   | When licence metadata for a versionless NuGet component was read from a release cdxgen picked, rather than one the project declared               | Traces where the description and licence came from without claiming the component is that release                                                                                                                                                                                                     | Context only     |
...
| `cdx:rusi:importedBy`                                                                     | purl of the package whose source imports this crate.   |
...
<a id="kosi-kotlin-evinse-evidence"></a>

#### Kosi Kotlin Evinse evidence

`cdx:kosi:*` properties are emitted by `evinse -l kotlin` when the `kosi` helper is available. They are split between the root metadata component, dependency components, rendered `cryptographic-asset` components, and services.
Use metadata-level values to judge the evidence run: helper and schema versions, the Kotlin version analysed, and the slice counters.
Use component-level values to judge dependency posture: data-flow categories and rules, per-component slice counts, whether a flow crosses a dependency boundary, how complete the recorded trace is, and whether the component's evidence survived call-graph reachability from the roots.
...
| Property                              | Meaning                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `cdx:kosi:degraded`                   | Present only when kosi does not trust its own counts. `no-jdk`: the resolved tier attached no JDK, so every `java.*` symbol resolved to nothing — a native binary has no `java.home` of its own and falls back to `JAVA_HOME`, the `java` launcher on `PATH`, then the conventional install roots. `kotlin-version`: a language-version mismatch coincided with heavy resolution fallout. Either way the counts below describe the run, not the code. |
...
- **ONNX artifact metadata:** `cdx:onnx:irVersion`, `cdx:onnx:producer`, `cdx:onnx:producerVersion`, `cdx:onnx:opsets`
...
- `cdx:gguf:shardCount` present + fewer observed GGUF file components than expected
  | `cdx:onnx:irVersion`, `cdx:onnx:producer`, `cdx:onnx:producerVersion`, `cdx:onnx:opsets` | model component | enum/string | `9`, `pytorch`, `2.4.0`, `ai.onnx:17` | When an ONNX file's protobuf header is read without loading tensors | Ties the artifact to the exporting toolchain and operator sets, supporting provenance review when absent (rule AIS-004) | Context only |
