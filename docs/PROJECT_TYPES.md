...
| MCP inventory      | `mcp`                                                                                  | JavaScript / TypeScript MCP source trees, `.vscode/mcp.json`, `.mcp.json`, `claude_desktop_config.json`, `opencode.json`, `opencode.jsonc`                                            | Yes, for source-detected MCP packages/services plus MCP client configuration files                    | ✅                   |
| AI skill inventory | `ai-skill`, `skill`, `skills`                                                          | `CLAUDE.md`, `AGENTS.md`, `.github/copilot-instructions.md`, `.github/workflows/copilot-setup-steps.yml`, `SKILL.md`, `.opencode/**`, `.nanocoder/**`, `langgraph.json`, CrewAI files | Yes, for shipped AI instruction, skill, agent, and tool definitions discovered from supported layouts | ✅                   |
| AI / AI-BOM        | `ai`, `aibom`, `ai-bom`                                                                | Hugging Face purls / URLs, model repositories, `Modelfile`, `.gguf`, plus AI-aware JavaScript / TypeScript / Python repos and notebooks                                               | Yes, for discovered AI models, services, prompt/config artifacts, lineage, and formulation evidence   | ✅                   |

> JavaScript scans can also opt into AI asset and provider inventory by using `--bom-audit-categories ai-agent` or the broader `ai-inventory` alias. This adds CycloneDX AI components/services for detected model usage, Hugging Face references, Modelfiles, and GGUF artifacts.

> Use `-t ai` when you want AI-BOM inventory as an explicit project type. The dedicated `aibom` command is the same focused mode with `--include-formulation` enabled and `ai-bom` preselected as the default BOM-audit category alias.
...
| Java (Specific version) | `java8`, `java11`, `java17`, `java21`, `java22`, `java23`, `java24`, `java25`, `java26` | `pom.xml` [1], `build.gradle`, `.kts`, `sbt`, `bazel` | Yes, unless `pom.xml` is manually parsed due to unavailability of maven or errors) | ✅ |
...
| Python (Default) | `python`, `py`, `pypi`, `uv`, `pip`, `poetry`, `pdm`, `hatch` | `pyproject.toml`, `setup.py`, `requirements.txt` [2], `Pipfile.lock`, `poetry.lock`, `pdm.lock`, `uv.lock`, `pylock.toml`, `pylock.<name>.toml`, `bdist_wheel`, `.whl`, `.egg-info` | Yes using the automatic pip install/freeze. When disabled, only with `Pipfile.lock`, `pdm.lock`, `uv.lock`, `pylock.toml`, and `poetry.lock` | ✅ |
| Python (Specific version) | `python36`, `python38`, `python39`, `python310`, `python311`, `python312` | `pyproject.toml`, `setup.py`, `requirements.txt` [2], `Pipfile.lock`, `poetry.lock`, `pdm.lock`, `uv.lock`, `pylock.toml`, `pylock.<name>.toml`, `bdist_wheel`, `.whl`, `.egg-info` | Yes using the automatic pip install/freeze. When disabled, only with `Pipfile.lock`, `pdm.lock`, `uv.lock`, `pylock.toml`, and `poetry.lock` | - |
| Golang | `go`, `golang` | `binary`, `go.mod`, `go.sum`, `Gopkg.lock` | Yes except binary | ✅ |
| Rust | `rust`, `rust-lang`, `cargo` | `binary`, `Cargo.toml`, `Cargo.lock` | `Cargo.lock`, Cargo workspace manifests, and Cargo formulation/build metadata | ✅ |
| Rust (Cargo Cache) | `cargo-cache`, `cargo-index` | `$CARGO_HOME/registry/cache/\*\*/\*.crate`, `$HOME/.cargo/registry/cache/\*\*/\*.crate` | Filename-derived package identity plus SHA-256 from cached crate archives | - |
...
| .NET (C#, VB, F#) | `csharp`, `netcore`, `dotnet`, `vb`, `vbnet`, `visualbasic`, `f#`, `fs`, `fsharp`, `dotnet-framework` | `.sln`, `.slnx`, `.csproj`, `.vbproj`, `.fsproj`, `.tsproj`, `.plcproj`, `.hmiproj`, `packages.config`, `project.assets.json` [3], `packages.lock.json`, `.nupkg`, `paket.lock`, `binary` | Yes with `--deep` via dosai for occurrence evidence and API services; cdxgen/evinse `--profile research` adds crypto and data-flow evidence | - |
...
| C++ | `c`, `cpp`, `c++`, `conan`, `collider` | `collider.lock`, `conan.lock`, `conanfile.txt`, `\*.cmake`, `CMakeLists.txt`, `meson.build`, codebase without package managers! | Yes only for `collider.lock`, `conan.lock`. Best effort basis for `cmake` without version numbers. | ✅ |
...
| Hardware BOM | `hbom`, `hardware` | live supported hosts via `@cdxgen/cdx-hbom` (`darwin/arm64`, `linux/amd64`, `linux/arm64`) | native hardware evidence collected from supported host commands and system interfaces | - |
| Operating System (OS) | `os`, `osquery`, `windows`, `linux`, `mac`, `macos`, `darwin` |
| Jenkins Plugins | `jenkins` | `.hpi files` | - | ✅ |
| VS Code Extensions | `vscode-extension`, `vsix`, `vscode`, `openvsx`, `vscode-extensions`, `ide-extensions` | `.vsix`, installed extension directories, IDE auto-discovery [6] | Yes | - |
| Chrome Extensions | `chrome-extension`, `chrome-extensions`, `chromium-extension`, `chromium-extensions` | Installed Chrome/Chromium-based browser extension directories (Chrome, Chromium, Edge, Brave, Vivaldi) [7] | Yes | - |
| Electron ASAR Archives | `asar`, `electron`, `electron-asar` | `.asar` archives with native file inventory, hashes, JS capability analysis, and embedded Node manifests [10] | Yes | ✅ |
| Helm | `helm`, `charts` | `.yaml` | n/a | |
| Helm (Cache) | `helm-index`, `helm-repo` | `$HOME/.cache/helm/repository/\*\*/\*.yaml` | - | - |
| Container | `oci`, `docker`, `podman`, `container`, `rootfs`, `oci-dir` | n/a | - |
...
> - `hbom` is a dedicated host inventory mode. Do not mix it with SBOM/OBOM project types in the same invocation; generate HBOM separately using `hbom` or `cdxgen -t hbom`.
...
> - For .NET projects, deep evidence uses the optional `dosai` binary bundled through `@cdxgen/cdxgen-plugins-bin`. Use the cdxgen container images or install a compatible .NET SDK when local dosai execution is required.
...
- [10] - ASAR support reads archive headers natively without requiring the Electron `asar` package. In normal mode cdxgen can extract the archive to a temp directory to reuse existing Node.js manifest parsing for embedded `package.json` and lockfiles. In `--dry-run` mode header parsing and file inventory still work, but temp extraction remains blocked and BOM-audit coverage for embedded package install scripts is therefore partial.
