...
| `applyMetadata()`                   | converts `internal:SrcFile` and evidence paths to relative values, then adds `cdx:bom:componentTypes`, `cdx:bom:componentNamespaces`, and `cdx:bom:componentSrcFiles` metadata properties  |
